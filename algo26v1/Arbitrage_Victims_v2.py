"""Algothon 2026 submission (v2) — market-neutral peer-lead-lag strategy.

Identical strategy to Arbitrage_Victims.py, with ONE robustness fix: the model
cache refit-guard is hardened so it can never silently serve a stale (future-fit)
model if getMyPosition is ever called out of strict day order. Behaviour on the
grader's monotonic day-by-day loop is bit-identical; the fix only removes a
lookahead trap under non-monotonic / repeated calls. ALGO risk unchanged.

Each day: predict every asset's next-day return from today's full cross-section
using a forgetting-weighted ridge (EWLS, half-life 500d, light L2 alpha=0.1),
...
Backtests to Score ~762 (last-250) / ~582 (full days 60-500) via eval.py.
"""
import numpy as np
...
def getMyPosition(prcSoFar):
    ...
    # Refit whenever the history length CHANGES (fit is keyed to exactly this t). The old guard
    # `t - last_fit_t >= 1` only refit on ADVANCE, so a non-monotonic / repeated call at a smaller t
    # would silently reuse a model fit on MORE (future) data -> lookahead. `fit_t != t` is safe both ways
    # and identical on the grader's monotonic loop (fit is ~1ms, so caching only dedupes same-t calls).
    if _cache["fit_t"] != t:
