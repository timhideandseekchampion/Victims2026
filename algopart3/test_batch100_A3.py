...
IDEA A3: Resweep BLEND (reversal-leg weight in the wz combination) against v10, e.g.
0.15, 0.2, 0.25, 0.3, 0.35, 0.4. Shipped value is 0.3.

BLEND only affects the linear recombination `wz = (1-BLEND)*ridge_ensemble + BLEND*REV`, applied
AFTER the (expensive) ridge ensemble and BEFORE the boost / rank-stability blend. The raw ridge
ensemble mean and REV leg are each precomputed ONCE (cheap loop after that), reused for every
candidate BLEND value; boost and ALGO leg are also independent and cached once.
"""
...
print("=== precompute: raw ridge ensemble mean, REV leg, pairwise boost, rank-stability signal, "
      "ALGO leg -- all independent of BLEND, cached once ===", flush=True)
t0 = time.time()
days = list(range(WARMUP, nt))

RAW = np.full((nIdio, nt), np.nan)  # ridge ensemble mean, BEFORE BLEND recombination
for t in days:
    rr_ = r[:, :t]
    X = rr_[:, :-1].T
...
print(f"\n=== sanity check: BLEND={SHIPPED_BLEND} (shipped) must reproduce SAFE_llboost_v10 exactly ===")
t0 = time.time()
POS_base = build_pos(SHIPPED_BLEND)
base_scs = scs_curve(POS_base)
base_wo, base_wn = wscore(POS_base, *OLD), wscore(POS_base, *NEW)
print(f"  BLEND={SHIPPED_BLEND}: OLD={base_wo:.1f}  NEW={base_wn:.1f}  rmean={base_scs.mean():.1f}  "
...
def evaluate(blend, verbose=True):
    Pz = build_pos(blend); scs = scs_curve(Pz)
    wo = wscore(Pz, *OLD); wn = wscore(Pz, *NEW)
    passed = (wo > base_wo) and (wn > base_wn) and (scs.mean() > base_scs.mean())
    nworse = int((scs < base_scs).sum())
    if verbose:
        tag = "  <== PASS" if passed else ""
        print(f"  BLEND={blend:<6}OLD={wo:7.1f}  NEW={wn:7.1f}  rmean={scs.mean():7.1f}  "
              f"rfloor={scs.min():7.1f}  n_worse={nworse}/{len(scs)}{tag}")
    return dict(b=blend, wo=wo, wn=wn, rm=scs.mean(), rf=scs.min(), nworse=nworse, passed=passed)


print(f"\n=== SWEEP: BLEND (shipped={SHIPPED_BLEND}) ===")
SWEEP = [0.15, 0.2, 0.25, 0.3, 0.35, 0.4]
t0 = time.time()
results = [evaluate(b) for b in SWEEP]
print(f"  sweep done ({time.time()-t0:.1f}s)")

passing = [c for c in results if c["passed"]]
print(f"\n{len(passing)}/{len(results)} BLEND values beat v10 on OLD+NEW+rmean jointly.")
best = max(results, key=lambda c: c["rm"])
print(f"Best by rolling mean: BLEND={best['b']} (rmean={best['rm']:.1f} vs shipped rmean={base_scs.mean():.1f})")
