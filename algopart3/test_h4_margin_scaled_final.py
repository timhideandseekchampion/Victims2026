"""H4 (plan: revisit the pairwise family): re-test continuous margin-scaled boost on the CURRENT
final config (scale_w=1000, IC_L=190, K=1.5), now that BOOST_MIN_DAY=500 removes the thin-sample
false-positive regime that made the earlier "gradual" test (test_boost_floor_mitigation.py section
3) fail. eff_k = K * min(margin, 2.0) where margin = |corr|/threshold. Compared directly against
the CURRENT shipped SAFE_llboost (774.1/828.6/811.4/563.8), not the older baseline.
"""
...
print("=== precompute (shared): shipped idio WZ, ALGO leg, day-by-day significant leader map + margin ===")
t0 = time.time()
WZ = {}
...
n = rs.shape[0]
LEADER_AT = {}  # day k -> {j: (boost_val, margin)}
t0 = time.time()
for k in range(BOOST_MIN_DAY, nt):
    ...
    for j in range(n):
        ...
        if ic <= 0:
            continue
        margin = abs(col[i]) / thr
        entry[j] = (lead_boost[-1], margin)
    LEADER_AT[k] = entry
print(f"  significant-leader+margin map done ({time.time()-t0:.0f}s)")

end_days = list(range(400, nt + 1, 10))
OLD = (500, 750); NEW = (750, nt)


def build_pos(K, mode, margin_cap=2.0):
    POS = np.zeros((nInst, nt))
    for k in range(SAFE.WARMUP, nt):
        cur = P_[:, k]; lim = (dlr / cur).astype(int)
        wz = WZ[k].copy()
        if k >= BOOST_MIN_DAY:
            for j, (bv, margin) in LEADER_AT[k].items():
                eff_k = K if mode == "hard" else K * min(margin, margin_cap)
                wz[j] += eff_k * bv
...
print("\n=== current shipped SAFE_llboost (hard gate, K=1.5) ===")
ship_scs = report("shipped SAFE_llboost (hard)", build_pos(BOOST_K, "hard"))

print("\n=== gradual/margin-scaled, sweep base K ===")
for K in (0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 4.0):
    report(f"gradual K={K}", build_pos(K, "gradual"), ship_scs)

print("\n=== gradual, sweep margin_cap at best-looking K ===")
for cap in (1.5, 2.0, 2.5, 3.0, 4.0):
    report(f"gradual K=2.0 cap={cap}", build_pos(2.0, "gradual", margin_cap=cap), ship_scs)
