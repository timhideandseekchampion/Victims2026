...
entries & exits + per-asset PnL across all days (day count read from the data)."""
import json
...
:root{
  ...
  --long:#0e9e73; --short:#e0455e; --pnl:#3b6fd4; --vo:#d98a2b;
...
@media (prefers-color-scheme:dark){:root{
  ...
  --long:#2fd39a; --short:#ff6b83; --pnl:#6fa0f5;--vo:#e0a24a;
  --longband:rgba(47,211,154,.15); --shortband:rgba(255,107,131,.13);
}}
:root[data-theme=dark]{--bg:#15171a;--surface:#1d2024;--ink:#e9eaec;--ink2:#a6adb5;--muted:#6b7178;--grid:#2a2e33;--border:#31363c;--long:#2fd39a;--short:#ff6b83;--pnl:#6fa0f5;--vo:#e0a24a;--longband:rgba(47,211,154,.15);--shortband:rgba(255,107,131,.13);}
...
<h1>SAFE vs QUAL vs SWING vs LLALGO vs LLDOLLAR vs LLVOL — entries &amp; exits, all 51 assets · __NT__ days</h1>
...
<div class="detailhead" style="margin-top:26px">
  <span class="nm">ALGO index leg — vol+momentum vs vol-only ($ position)</span>
  <span class="stat" id="algostat"></span>
</div>
<div class="card"><canvas id="algoleg" height="160"></canvas></div>
<p class="hint">The ALGO index (row 0) is the <b>only</b> leg where the two LLVOL books differ — instruments 1–49 are identical.
<b style="color:var(--pnl)">Blue = LLVOL (v+m)</b>, <b style="color:var(--vo)">amber = LLVOL·VO (vol-only)</b>. Above 0 = long ALGO, below = short.
Where blue runs bigger, the momentum leg is confirming the vol side (conviction up); where they sit on opposite sides of 0, momentum flipped the position. Independent of the strategy selector. Hover for both books' $.</p>
...
<p class="hint">Each dot is one asset: x = PnL over the first half of the traded window, y = second half (split at the midpoint day).
...
const WARMUP0=130, MID=Math.floor((WARMUP0+NT-1)/2);   // midpoint of the traded range (adapts to NT)
function renderPersist(){
  ...
  const pts=[];for(let i=1;i<N;i++){pts.push({i,x:pnl[i][MID]-pnl[i][WARMUP0],y:pnl[i][NT-1]-pnl[i][MID],nm:DATA.names[i]});}
...
// ---- ALGO leg overlay: LLVOL v+m vs vol-only $ position ----------------------
function renderAlgoLeg(){
  const cv=document.getElementById('algoleg'),[c,W,H]=dpi(cv,160),pad={l:62,r:14,t:12,b:22};
  ...
  c.clearRect(0,0,W,H);
  c.font="11px sans-serif";c.textAlign="right";c.lineWidth=1;
  for(const v of [m,m/2,0,-m/2,-m]){const y=ys(v);c.strokeStyle=v===0?css('--border'):css('--grid');
    c.beginPath();c.moveTo(pad.l,y);c.lineTo(W-pad.r,y);c.stroke();
    c.fillStyle=css('--muted');c.fillText((v>0?'+':'')+'$'+(v/1000)+'k',pad.l-6,y+3);}
  c.fillStyle=css('--muted');c.textAlign="center";for(const d of niceDayTicks(0,NT-1))c.fillText(d,xs(d),H-6);
  ...
  line(B,'--vo',1.5);line(A,'--pnl',1.9);
  let sa=0,sb=0,n=0;for(let d=130;d<NT;d++){sa+=Math.abs(A[d]);sb+=Math.abs(B[d]);n++;}
  document.getElementById('algostat').innerHTML=`mean |$|: <b style="color:var(--pnl)">$${Math.round(sa/n/1000)}k</b> v+m · <b style="color:var(--vo)">$${Math.round(sb/n/1000)}k</b> vol-only`;
  cv._geoA={pad,A,B};
}
(function(){const cv=document.getElementById('algoleg');cv.addEventListener('mousemove',e=>{const g=cv._geoA;if(!g)return;
  const r=cv.getBoundingClientRect();let d=Math.round((e.clientX-r.left-g.pad.l)/((cv.clientWidth-g.pad.l-g.pad.r))*(NT-1));d=Math.max(0,Math.min(NT-1,d));
...
// theme toggle (CSS already has [data-theme] rules; nothing sets the attribute without this)
(function(){
  ...
  btn.addEventListener('click',()=>{
    ...
    paint();
    renderDetail();renderSkew();renderPersist();renderGrid();renderAlgoLeg();
  });
})();
...
window.addEventListener('resize',()=>{renderDetail();renderSkew();renderPersist();renderGrid();renderAlgoLeg();});
renderDetail();renderSkew();renderPersist();renderGrid();renderAlgoLeg();
</script></body></html>"""

html = HTML.replace("__DATA__", json.dumps(DATA, separators=(",", ":"))).replace("__NT__", str(DATA["nt"]))
