"""
================================================================================
###  SAFE_llboost_v6.py  ·  SAFE_llboost_v5 + SAFE_llboost_v2 vol-regime-      ###
###                          adaptive momentum lookback (best result overall) ###
================================================================================
Combines v5 (N=39 boost candidate pool + re-tuned IC_L=250/MIN_DAY=480, n_worse=0/61 on its own)
with v2 (ALGO leg's momentum lookback switches MOM_LB_SHORT=7/MOM_LB_LONG=12 by vol regime,
n_worse=18/61 on its own) -- the same orthogonal-components logic validated for v4 (v3+v2).
  ...
  SAFE_llboost_v6 (this file)      811.4         868.9         857.0           669.5           9/61

Beats v4 on every metric (marginally) -- the IC_L/MIN_DAY refinement carries through the
combination cleanly. Validated on the actual getMyPosition pathway (eval_llboost_v6.py: official
score 868.87 exactly; validate_llboost_v6_full.py). Confirmed identical to v2 on days 100-400
(boost still inactive there in both: 576.0 in both) -- zero side effects from the boost changes
before they can possibly activate. Best result of the whole session.

See SAFE_llboost_v3.py, SAFE_llboost_v5.py, and SAFE_llboost_v2.py docstrings for each component's
...
BOOK = "SAFE · LL-BOOST v6 (v5 boost refinement + v2 vol-regime-adaptive momentum)"
