"""
================================================================================
###  SAFE_llboost_v5.py  ·  SAFE_llboost_v3 + re-tuned BOOST_IC_L/MIN_DAY     ###
================================================================================
From an 80-idea follow-up test batch (test_batch80_catA_boostpool.py): once the boost's leader
search pool is restricted to N=39 (v3), the OTHER boost sub-parameters (originally tuned against
the N=49/unrestricted pool) were re-swept to check whether they still sit at their optimum. Two
didn't: BOOST_IC_L=190 -> 250 and BOOST_MIN_DAY=500 -> 480 each independently improve slightly,
confirmed via a neighbor-stability check (IC_L in 230-270 all give n_worse=1/61; MIN_DAY in
470-490 all give n_worse=0/61 -- stable plateaus, not isolated spikes) and they compound:
  ...
  SAFE_llboost_v5 (this file)      796.6         839.1         828.3           563.8           0/61

A small refinement on top of v3 (not a new mechanism) -- same N=39 candidate-pool restriction,
just re-tuned IC_L/MIN_DAY for the new pool size. Achieves a clean n_worse=0/61, better than v3
on every metric. See SAFE_llboost_v3.py's docstring for the full N=39 pool-restriction rationale
and validation, which carries over unchanged here.
================================================================================
...
BOOK = "SAFE · LL-BOOST v5 (v3 candidate-pool restriction, re-tuned IC_L/MIN_DAY)"
...
BOOST_MIN_DAY = 480       # no boost at all before this many days of return history exist -- re-tuned
                          # from 500 given the N=39 pool (test_batch80_catA_boostpool.py), stable
                          # over 470-490 (n_worse=0/61 throughout)
...
BOOST_IC_L = 250          # trailing window (days) used for the leader-pair sign-of-edge check.
                          # Re-tuned from 190 given the N=39 pool (test_batch80_catA_boostpool.py),
                          # stable over 230-270 (n_worse=1/61 throughout -- combined with the
                          # MIN_DAY re-tune above, gives n_worse=0/61).
