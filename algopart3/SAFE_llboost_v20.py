"""
================================================================================
###  SAFE_llboost_v20.py  ·  v15 + a small orthogonal 'resid' diversifier blend  ###
================================================================================
Built 2026-07-30, testing a genuine signal already on record in this repo's memory but never
actually tried in the book: the Avellaneda-Lee-lite index-residual reversion challenger ('resid')
from algopart2/SAFE_rotate.py, RESID_K=60 -- per-stock beta to the index estimated over a SHORT
trailing 60-day window, fading the cumulative index-neutral residual return over that same window.
Memory recorded it as "real IC +0.0195/t2.5 AND nearly orthogonal to champ (corr 0.07)" against a
now-superseded lineage -- re-verified FRESH against the current champion before building anything:
IC +0.0168/t+2.83 (still significant), corr with current champion +0.058 (still genuinely low).
Genuinely orthogonal signals with real IC are the textbook way to improve risk-adjusted return
without fighting the main signal -- this is NOT a replacement for anything, just a small blended
tilt, same spirit as the existing rank-stability blend.

MECHANISM CHANGE from v15: `_resid_signal` ported verbatim from algopart2/SAFE_rotate.py's `resid`
challenger. Blended into `_idio_signal` right after the rank-stability blend (same pattern:
z-scored, weighted by RESID_WEIGHT, scaled to the champion signal's own average magnitude) --
before the post-jump fade, which stays last as the final discrete overlay. RESID_WEIGHT is swept
in `test_v20_resid_diversifier.py` against the real-data bar (OLD/NEW/rmean/n_worse vs v15) rather
...
BOOK = "SAFE · LL-BOOST v20 (v15 + small orthogonal resid diversifier blend)"
...
# --- NEW in v20: small orthogonal 'resid' index-residual reversion diversifier (ported verbatim
#     from algopart2/SAFE_rotate.py's 'resid' challenger) ---
RESID_K = 60
RESID_WEIGHT = 0.02   # first pass -- swept in test_v20_resid_diversifier.py
...
def _resid_signal(prcSoFar):
    """Avellaneda-Lee-lite index-residual reversion, ported verbatim from
    algopart2/SAFE_rotate.py's 'resid' challenger: per-stock beta to the index estimated over a
    SHORT trailing RESID_K=60 day window (distinct from the champion's own long-window
    BETA_DEMEAN_W=500 beta-adjustment), fading the cumulative index-neutral residual return over
    that same short window. Returns None if there isn't enough history yet."""
...
def _pairwise_boost(rs):
    """Identical to SAFE_llboost_v8/v9/v10/v11/v12._pairwise_boost -- plain, undecayed,
    full-history-only candidate selection. Deliberately NOT SAFE_llboost_v13/v14's gated
    decayed-selection fallback (see module docstring for why)."""
    n, T = rs.shape
    boost = np.zeros(n)
    if T < BOOST_MIN_DAY:
        return boost
...
def _idio_signal(prcSoFar):
    ...
    resid_sig = _resid_signal(prcSoFar)
    if resid_sig is not None:
        r_std = resid_sig.std()
        r_z = (resid_sig - resid_sig.mean()) / (r_std + 1e-12) if r_std > 1e-12 else np.zeros_like(resid_sig)
        wz = (1 - RESID_WEIGHT) * wz + RESID_WEIGHT * r_z * (np.abs(wz).mean() + 1e-12)
