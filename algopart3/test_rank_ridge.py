"""New idea, never tried this session: replace RAW returns with CROSS-SECTIONAL RANKS as the
ridge's inputs/targets, a standard robustness technique (Spearman-style) against fat-tailed outlier
days that could disproportionately swing a fitted coefficient even under ridge regularization.
Ranks are computed causally (using each day's own 49-stock cross-section, no look-ahead across
days), scaled to have similar variance to the original z-scored features so RIDGE_A stays
comparable. Reuses the fixed significance-boost + ALGO leg precompute.
"""
...
print("=== precompute (fixed): significance-gated boost + ALGO leg ===")
t0 = time.time()
n = rs.shape[0]
BOOST_AT = {}
...
print("computing cross-sectional rank transform of returns (per day, across all 51 instruments) ...")
T_ = r.shape[1]
r_rank = np.zeros_like(r)
for t in range(T_):
    ranks = pd.Series(r[:, t]).rank().values  # 1..51
    r_rank[:, t] = (ranks - ranks.mean()) / (ranks.std() + 1e-12)  # z-scored rank, comparable scale to r
print("done")
...
def build_pos(use_rank=False, ridge_a=SAFE.RIDGE_A):
    src = r_rank if use_rank else r
    ...
    for t in range(SAFE.WARMUP, nt):
        ...
        wz = np.mean(fs, 0)
        if SAFE.BLEND > 0:
            # reversion leg stays on RAW log-price levels regardless (measures actual price move)
...
print("\n=== baseline: raw-return ridge (current shipped) ===")
base_scs = report("raw returns (shipped)", build_pos(use_rank=False))

print("\n=== rank-transformed ridge inputs, sweep RIDGE_A (ranks have different natural scale) ===")
for ra in (0.03, 0.05, 0.1, 0.15, 0.2, 0.3, 0.5):
    report(f"rank ridge_a={ra}", build_pos(use_rank=True, ridge_a=ra), base_scs)
