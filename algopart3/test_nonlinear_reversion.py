"""New idea: the ridge's BLEND reversion component currently uses a plain LINEAR z-score
(rv = -z). But this session already validated that a power-law magnitude transform works well for
this dataset -- that's exactly the boost's sign(x)*(|x|/scale)^P construction, with P=2.0 at a
genuine optimum. Never tested: does applying the SAME kind of nonlinear transform to the
reversion leg (rv = -sign(z)*|z|^P) help, instead of the current linear (P=1) version?
"""
...
print("\n=== baseline: linear reversion (rev_p=1.0, current shipped) ===")
base_scs = report("linear rev (P=1.0, shipped)", build_pos(rev_p=1.0))

print("\n=== sweep reversion power exponent ===")
for p in (0.5, 0.75, 1.0, 1.25, 1.5, 2.0, 2.5, 3.0):
    report(f"rev_p={p}", build_pos(rev_p=p), base_scs)
