"""
================================================================================
###  SAFE_llboost_v24.py  ·  v22 + volatility-scaled ("risk parity") idio sizing  ###
================================================================================
Built 2026-07-30. A portfolio-construction change, not a new alpha signal: every idio name is
currently sized to the exact same flat $10k notional regardless of its own volatility, so a jumpy
name and a calm one contribute very different amounts of risk to the book even though they get the
same dollar weight. Since the score function directly rewards Sharpe (SR^2/(SR^2+1)), scaling each
name's dollar size inversely by its own trailing realized vol (relative to the cross-sectional
median, clipped to [VOL_SCALE_MIN, VOL_SCALE_MAX] so no name gets an extreme lever) should even out
risk contribution across the book without touching the forecast at all. VOL_SCALE_ON=False (or a
clip range of [1,1]) reduces exactly to v22's flat sizing. The final position is still hard-clipped
to the same per-instrument $ limit as every other file here, so this can never exceed the existing
notional cap even where the scale factor would push above 1.0x.

See test_v24_vol_scale.py for the sweep (window x clip-range) against the real-data bar (incl.
WIN250=day 250-500).
================================================================================
"""
import numpy as np
...
BOOK = "SAFE · LL-BOOST v24 (v22 + vol-scaled idio sizing)"

# --- vol-scaled ("risk parity") idio position sizing ---
VOL_SCALE_ON      = True
VOL_SCALE_WIN     = 60
VOL_SCALE_MIN     = 0.5
VOL_SCALE_MAX     = 2.0
VOL_SCALE_MIN_DAY = 200


def _vol_scale(prcSoFar):
    """Per-name dollar-size multiplier: cross-sectional-median-relative inverse of each name's own
    trailing VOL_SCALE_WIN-day realized idio vol, clipped to [VOL_SCALE_MIN, VOL_SCALE_MAX]. Causal
    -- uses only prcSoFar. Returns an all-ones vector (no-op) before enough history exists."""
    logp = np.log(prcSoFar)
    r = logp[1:, 1:] - logp[1:, :-1]
    n = r.shape[0]
    if r.shape[1] < VOL_SCALE_WIN:
        return np.ones(n)
    vol = r[:, -VOL_SCALE_WIN:].std(axis=1)
    ok = vol > 1e-12
    if not ok.any():
        return np.ones(n)
...
def getMyPosition(prcSoFar):
    ...
    if not killed:
        scale = (_vol_scale(prcSoFar) if (VOL_SCALE_ON and t >= VOL_SCALE_MIN_DAY)
                 else np.ones(nInst - 1))
        pos[1:] = np.sign(wz) * (dlr[1:] * scale / cur[1:])
