...
print("\nsanity check: full_shipped_score on REAL prices.txt must reproduce the known eval_llboost.py score (828.60)")
import time
t0 = time.time()
...
print("\n=== SYNTHETIC SCORE DISTRIBUTION (SAFE_llboost.py, N={} independent redraws) ===".format(N_DRAWS))
...
print(f"% of draws with score > 828 (our real-file score): {(synth_scores > 828).mean()*100:.0f}%")
print(f"% of draws with score > 900 (reported top-team range): {(synth_scores > 900).mean()*100:.0f}%")
print(f"% of draws with score > 930 (user's qualifying-safety target): {(synth_scores > 930).mean()*100:.0f}%")
print(f"our real prices.txt score (828.60) sits at percentile: "
      f"{(synth_scores < 828.60).mean()*100:.0f} of the synthetic distribution")
