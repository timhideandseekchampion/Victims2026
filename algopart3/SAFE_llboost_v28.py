"""
================================================================================
###  SAFE_llboost_v28.py  ·  v22 + learned per-half-life ridge ensemble weight  ###
================================================================================
Built 2026-07-30. v22's ridge ensemble combines its 4 EWLS half-lives (250/500/1000/2000) via a flat
equal-weighted mean. This replaces that with a weight learned from each half-life's OWN trailing
predictive quality -- POOLED ACROSS ALL 50 NAMES (not per-name): per-name significance testing was
just tried for a deadband idea and failed catastrophically (Bonferroni-corrected across 50
simultaneous names, at this sample size, is close to unsatisfiable -- 42/50 names never cleared it
even once in 601 days). Pooling across all 50 names for each of the 4 half-lives avoids that trap:
~50x the effective sample size per test, and only 4 comparisons instead of 50.

mult_hl(t) = clip(1 + RIDGE_LEARN_GAIN*pooled_ic_hl(t), 0, RIDGE_LEARN_CAP)
w_hl(t) = mult_hl(t) / sum(mult), wz0(t) = sum_hl(w_hl(t) * fi_hl(t))

pooled_ic_hl(t) is half-life hl's own standalone z-scored forecast vs realized return, correlated
over the trailing RIDGE_LEARN_WIN days, flattened across all 50 names. Falls back to flat 1/4
weights before enough history exists. Reuses v22's two-hop boost and G84 learned RS weight verbatim
(both independent of how the ridge ensemble is combined).

Ported from a dense (gain x cap x window) sweep, test_v27_ridge_weights.py: at
RIDGE_LEARN_GAIN=2.0, RIDGE_LEARN_CAP=2.0, RIDGE_LEARN_WIN=250, real prices.txt WIN250(250-500) and
OLD(500-750) tied exactly to v22, NEW +0.3, rmean +0.1, n_worse=0/61 -- modest but clean, same
plateau-then-tradeoff shape (gain 1-2 flat/safe, gain 3 bigger but noisier at n_worse=25/61, gain 5
fails OLD) as every other validated candidate this session.

See test_v28_ridge_weights_live.py for the byte-identical-to-precompute verification and the full
real-data/change-point/trend-regime battery.
================================================================================
"""
import numpy as np
from scipy import stats

BOOK = "SAFE · LL-BOOST v28 (v22 + learned per-half-life ridge weight)"

# --- learned per-half-life ridge ensemble weight (new in v28) ---
RIDGE_LEARN_GAIN = 2.0
RIDGE_LEARN_CAP  = 2.0
RIDGE_LEARN_WIN  = 250
...
_FI_HIST = {}   # n -> (4, 50) array, each half-life's own standalone z-scored ridge forecast
_RAWRET  = {}   # n -> RAW (non-demeaned) realized idio return over day n -- own cache since the
                # ridge-weight learning needs RIDGE_LEARN_WIN(=250)-day lookback, much longer than
                # _RET's own LOOKBACK(=71)-day retention window
_RIDGE_CACHE_LOOKBACK = RIDGE_LEARN_WIN + PRUNE_PAD
RIDGE_LEARN_MIN_DAY = max(WARMUP + RIDGE_LEARN_WIN, 400)
...
def _pooled_ridge_weight(n):
    """Learned per-half-life ridge-combine weight: pooled (across ALL 50 names, not per-name --
    per-name Bonferroni-corrected testing at this sample size was just shown to collapse) trailing
    RIDGE_LEARN_WIN-day IC of each half-life's own standalone forecast vs realized return.
    Falls back to flat 1/(len(HALF_LIVES)) weights before enough history exists."""
    n_hl = len(HALF_LIVES)
    if n < RIDGE_LEARN_MIN_DAY:
        return np.full(n_hl, 1.0 / n_hl)
    lo = n - RIDGE_LEARN_WIN
    fi_stack = np.stack([_FI_HIST[k] for k in range(lo, n)], axis=2)   # (n_hl, 50, WIN)
    ret_stack = np.stack([_RAWRET[k] for k in range(lo, n)], axis=1)   # (50, WIN)
    ys = ret_stack.ravel()
    mult = np.ones(n_hl)
    for hi in range(n_hl):
        xs = fi_stack[hi].ravel()
        ok = np.isfinite(xs) & np.isfinite(ys)
        if ok.sum() < 200 or xs[ok].std() < 1e-12:
            continue
        ic = float(np.corrcoef(xs[ok], ys[ok])[0, 1])
        mult[hi] = np.clip(1.0 + RIDGE_LEARN_GAIN * ic, 0.0, RIDGE_LEARN_CAP)
    if mult.sum() < 1e-6:
        return np.full(n_hl, 1.0 / n_hl)
...
def _idio_signal(prcSoFar):
    """The champion's FULL, final idio forecast: ridge ensemble (learned per-half-life weight, v28)
    + beta-adjusted target, BLEND reversion, two-hop transitive pairwise boost (v19), learned
    per-name rank-stability blend weight (v21), post-jump fixed-size fade (v12, applied last).
    Returns a 50-vector (idio names only, ALGO excluded)."""
    ...
    _FI_HIST[n] = fs
    if n >= 2 and (n - 1) not in _RAWRET:
        _RAWRET[n - 1] = logp[1:, -1] - logp[1:, -2]
    for k in [k for k in _FI_HIST if k < n - _RIDGE_CACHE_LOOKBACK]:
        del _FI_HIST[k]
    for k in [k for k in _RAWRET if k < n - _RIDGE_CACHE_LOOKBACK]:
        del _RAWRET[k]

    w = _pooled_ridge_weight(n)
    wz = (w[:, None] * fs).sum(0)
    if BLEND > 0:
