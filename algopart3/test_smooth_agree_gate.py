"""Fix for the discrete agree/disagree gate's jaggedness (demonstrated in
test_ic_ew_w_discreteness_demo.py): replace the hard step `(ice>=0)==(icf>=0) -> full size else 0`
with a smooth sigmoid weight based on icf*ice (positive=agree, negative=disagree), so a handful of
near-zero "coin flip" days blend smoothly instead of jumping between full-size and flat. Reuses the
shared fixed idio-book (ridge+boost) precompute; only the ALGO leg's _side() gate changes.
"""
...
IC_EW_HL, IC_EW_W, MOM_LB, COMBINE_GAIN = (20, 45), 200, 10, 3.5


def algo_vol_shares_smooth(lpA, cur0, cap_dol, smooth_k):
    """smooth_k=None -> exact original discrete gate (sanity check point). Otherwise sigmoid weight
    on icf*ice replaces the hard agree/disagree step."""
    T_ = len(lpA)
    ...
    def _side(feat, fhv):
        ...
        agree_score = icf * ice
        weight = 1.0 / (1.0 + np.exp(-smooth_k * agree_score))
        return sf * fhv * weight
...
def build_algo(smooth_k):
    algo_pos = np.zeros(nt)
    for k in range(130, nt):
        cur0 = P_[0, k]; lim0 = int(dlr[0] / cur0)
        algo_pos[k] = np.clip(algo_vol_shares_smooth(logp[0, :k + 1], cur0, dlr[0], smooth_k), -lim0, lim0)
    return algo_pos
...
print("\n=== sanity check: smooth_k=None must reproduce the exact discrete-gate baseline ===")
base_scs = report("discrete gate (smooth_k=None)", full_pos(build_algo(None)))

print("\n=== sweep smoothing steepness (smaller k = smoother/more gradual transition) ===")
for k in (5, 10, 20, 50, 100, 200, 500, 1000):
    report(f"smooth_k={k}", full_pos(build_algo(k)), base_scs)

print("\n=== push higher: find where smoothing converges back to the discrete baseline ===")
for k in (2000, 5000, 10000, 50000, 200000):
    report(f"smooth_k={k}", full_pos(build_algo(k)), base_scs)
