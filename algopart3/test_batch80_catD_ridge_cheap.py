"""Batch of 80, Category D (items 61-80): ridge/ensemble estimation variants + cheap checks.
"""
...
print("=== shared precompute: significance boost (fixed) + ALGO leg (fixed) ===")
t0 = time.time()
BOOST_AT = {}
...
print("\n### Item 65: separate RIDGE_A per half-life (instead of one shared value) ###")
def wz_perhl_ridge_a(t, a_map):
    rr = r[:, :t]
    fs = []
    for hl in SAFE.HALF_LIVES:
        B, mx, my = SAFE._ewls_ridge(rr[:, :-1].T, rr[1:, 1:].T, hl, a_map[hl])
        pred = my + (rr[:, -1] - mx) @ B
        fi = pred - pred.mean()
        fs.append(fi / (fi.std() + 1e-12))
    wz = np.mean(fs, 0)
...
for a_short, a_long in [(0.05, 0.1), (0.1, 0.2), (0.15, 0.05), (0.2, 0.1)]:
    a_map = {250: a_short, 500: a_short, 1000: a_long, 2000: a_long}
    report(f"per-HL RIDGE_A short={a_short},long={a_long}", combine_and_score(lambda t, am=a_map: wz_perhl_ridge_a(t, am)), base_scs)

print("\n### Item 69/70: trimmed-mean / median ensemble across half-lives (instead of simple mean) ###")
def wz_robust_ensemble(t, mode):
    rr = r[:, :t]
    fs = []
    for hl in SAFE.HALF_LIVES:
        B, mx, my = SAFE._ewls_ridge(rr[:, :-1].T, rr[1:, 1:].T, hl, SAFE.RIDGE_A)
    ...
    fs = np.array(fs)  # (4, 50)
    if mode == "median":
        wz = np.median(fs, axis=0)
    elif mode == "trimmed":
        # drop max and min per-stock, average remaining 2
        srt = np.sort(fs, axis=0)
        wz = srt[1:3].mean(axis=0)
    if SAFE.BLEND > 0:
        rv_ = logp[1:, t] - logp[1:, t - SAFE.REV_W]
        rv_ = rv_ - rv_.mean()
        rv = -rv_ / (rv_.std() + 1e-12)
...
report("median ensemble (4 HLs)", combine_and_score(lambda t: wz_robust_ensemble(t, "median")), base_scs)
report("trimmed-mean ensemble (drop max/min)", combine_and_score(lambda t: wz_robust_ensemble(t, "trimmed")), base_scs)

print("\n### Item 67/68: extend HALF_LIVES ensemble with an extra very-short/very-long member ###")
def wz_extra_hl(t, extra_hls):
    rr = r[:, :t]
    fs = []
    for hl in list(SAFE.HALF_LIVES) + extra_hls:
        B, mx, my = SAFE._ewls_ridge(rr[:, :-1].T, rr[1:, 1:].T, hl, SAFE.RIDGE_A)
        pred = my + (rr[:, -1] - mx) @ B
        fi = pred - pred.mean()
        fs.append(fi / (fi.std() + 1e-12))
...
report("HL ensemble + extra HL=100", combine_and_score(lambda t: wz_extra_hl(t, [100])), base_scs)
report("HL ensemble + extra HL=4000", combine_and_score(lambda t: wz_extra_hl(t, [4000])), base_scs)
report("HL ensemble + extra HL=100,4000", combine_and_score(lambda t: wz_extra_hl(t, [100, 4000])), base_scs)

print("\nCategory D batch 1 (ridge estimation variants) complete.")
