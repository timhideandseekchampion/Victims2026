"""Sweep IC_EW_W (currently 200) -- the max lookback window for the ALGO leg's fast recency-
weighted IC (_ic_ew), used in the switch's fast/slow-agree gate. Every other ALGO leg parameter was
just re-swept against the current boosted idio book; this one was left at default. Reuses the
shared WZ + significance-boost precompute (fixed idio side) + the parameterized algo_vol_shares
from test_algo_leg_resweep.py.
"""
...
print("\n=== baseline: current shipped SAFE_llboost (IC_EW_W=200) ===")
base_scs = report("shipped (IC_EW_W=200)", full_pos(build_algo()))

print("\n=== sweep IC_EW_W ===")
for v in (100, 150, 175, 200, 225, 250, 300, 400):
    report(f"IC_EW_W={v}", full_pos(build_algo(IC_EW_W=v)), base_scs)

print("\n=== neighbor-stability check around IC_EW_W=150 ===")
for v in (120, 130, 140, 145, 150, 155, 160, 170):
    report(f"IC_EW_W={v}", full_pos(build_algo(IC_EW_W=v)), base_scs)
