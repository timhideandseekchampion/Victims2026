"""Option 2: test whether smoothing the short/long MOM_LB switch (currently a hard jump the
instant volz crosses zero) improves robustness -- blend the short-lookback and long-lookback
momentum z-scores continuously based on |volz| magnitude, instead of a hard either/or choice.
"""
...
def algo_vol_shares_smooth_switch(lpA, cur0, cap_dol, smooth_width):
    """smooth_width=0 -> exact hard switch (sanity check point). Otherwise blend short/long
    lookback momentum z-scores continuously based on volz magnitude via a sigmoid."""
    T_ = len(lpA)
    ...
    if smooth_width <= 0:
        ...
        z_short = z10_for(SHORT_LB); z_long = z10_for(LONG_LB)
        # blend the FULL historical series element-wise (not just today), so _side()'s internal
        # trailing-IC checks have valid history to work with -- same fix as the ensemble-MOM_LB bug.
        w_short_hist = 1.0 / (1.0 + np.exp(-volz / smooth_width))
        z10 = w_short_hist * z_short + (1 - w_short_hist) * z_long
        both_nan = np.isnan(z_short) | np.isnan(z_long) | np.isnan(volz)
        z10[both_nan] = np.nan

    fhm = np.clip(z10[tnow], -3, 3) / 3.0 if not np.isnan(z10[tnow]) else np.nan
...
def build_algo(smooth_width):
    algo_pos = np.zeros(nt)
    for k in range(130, nt):
        cur0 = P_[0, k]; lim0 = int(dlr[0] / cur0)
        algo_pos[k] = np.clip(algo_vol_shares_smooth_switch(logp[0, :k + 1], cur0, dlr[0], smooth_width), -lim0, lim0)
    return algo_pos
...
print("\n=== sanity: smooth_width=0 must reproduce the exact hard-switch (v2) baseline ===")
base_scs = report("hard switch (v2, smooth_width=0)", full_pos(build_algo(0)))

print("\n=== smooth transition, sweep width (smaller = sharper/closer to hard switch) ===")
for w in (0.05, 0.1, 0.2, 0.3, 0.5, 0.75, 1.0, 1.5):
    report(f"smooth_width={w}", full_pos(build_algo(w)), base_scs)
