"""
test_v19_twohop.py -- validates SAFE_llboost_v19 (v15 + two-hop transitive boost, C43 re-tested
fresh against the CURRENT baseline rather than trusting the stale v10-era README numbers) against
real data, change-point reverse/rotate, and trend-regime momentum/flip/noise -- same convention as
test_v15_insurance_only.py / test_v16_ic_gate.py.
...
Run: python3 test_v19_twohop.py
"""
...
def real_data_check():
    print("=== 1. real prices.txt: v19 vs v15 ===")
    ...
    curve19 = np.array([wscore(POS19, P_, E - NUMTEST, E, nInst) for E in end_days])
    n_worse = int((curve19 < curve15).sum()); n_better = int((curve19 > curve15).sum())
    mism = int((~np.all(POS15 == POS19, axis=0)).sum())
    ...
    print(f"  v19: OLD={wscore(POS19,P_,500,750,nInst):.1f} NEW={wscore(POS19,P_,750,nt,nInst):.1f} "
          f"rmean={curve19.mean():.1f} rfloor={curve19.min():.1f}")
...
def run_changepoint(mode, seed):
    ...
    reset_module_state(V19); POS19 = walk_pos_idio(V19, out, NT_PRE)
    ...
    pnl19 = daily_pnl_idio(POS19, out, NT_PRE + 1, nt - 1)
    pnl_oracle = daily_pnl_idio(oracle_POS_full, out, NT_PRE + 1, nt - 1)

    cum10 = np.cumsum(pnl10)[-1]; cum15 = np.cumsum(pnl15)[-1]; cum19 = np.cumsum(pnl19)[-1]
    cum_oracle = np.cumsum(pnl_oracle)[-1]
    loss10 = cum_oracle - cum10; loss15 = cum_oracle - cum15; loss19 = cum_oracle - cum19
    saved15 = (loss10 - loss15) / max(1.0, loss10) * 100
    saved19 = (loss10 - loss19) / max(1.0, loss10) * 100
    mism = int((~np.all(POS15 == POS19, axis=0)).sum())
    print(f"  mode={mode:8s} seed={seed}: v10={cum10:9.0f}  v15={cum15:9.0f}  v19={cum19:9.0f}  "
          f"oracle={cum_oracle:9.0f}  saved15={saved15:5.1f}%  saved19={saved19:5.1f}%  "
          f"days v15!=v19={mism}")
    return saved19


def trend_regime():
    print("\n=== 3. trend-regime momentum/flip/noise (v19 vs v10/v15) ===")
    ...
    for kind in ("momentum", "flip", "noise"):
        ...
        reset_module_state(V19); POS19 = walk(V19, full, S, E)
        pnl10 = daily_pnl(POS10, full, S, E)
        pnl19 = daily_pnl(POS19, full, S, E)
        print(f"  {kind:9s}: v10={pnl10.sum():9.0f}  v19={pnl19.sum():9.0f}")


if __name__ == "__main__":
    real_data_check()

    print("=== 2. change-point experiment: v19 vs v15 (vs recorded v15 numbers) ===")
    results = {"reverse": [], "rotate": []}
    for mode in ("reverse", "rotate"):
        for seed in SEEDS:
            results[mode].append(run_changepoint(mode, seed))
    print("\n=== summary: v19 frac of oracle-gap recovered vs plain v10 ===")
    for mode in ("reverse", "rotate"):
        vals = results[mode]
        print(f"{mode}: v19 frac_saved per seed = {[round(v,1) for v in vals]}  mean={np.mean(vals):.1f}%")
