"""Sweep BOOST_ALPHA (the pairwise boost's significance level, currently 0.05, Bonferroni-corrected
for 49 candidate leaders) -- never swept, just assumed. Looser alpha lets more (less certain) pairs
qualify; stricter alpha lets fewer (more certain) pairs through. Reuses the shared WZ + ALGO leg
precompute; only the significance threshold (and hence which pairs qualify) varies per alpha.
"""
...
print("=== precompute (shared): shipped idio WZ, ALGO leg ===")
t0 = time.time()
WZ = {}
...
def compute_boost_at(alpha):
    ...
    for k in range(BOOST_MIN_DAY, nt):
        ...
        thr = sig_threshold(n_samples, alpha)
        C = corrmat(Xi, Yj)
        entry = {}
        for j in range(n):
        ...
        boost_at[k] = entry
        avg_qualify.append(len(entry))
    return boost_at, np.mean(avg_qualify)
...
for alpha in (0.20, 0.10, 0.05, 0.02, 0.01, 0.005, 0.001):
    t0 = time.time()
    boost_at, avg_q = compute_boost_at(alpha)
    POS = build_pos(boost_at)
    label = f"alpha={alpha}" + (" (current)" if alpha == 0.05 else "")
    scs = report(f"{label:<24} avgQ={avg_q:.1f}", POS, base_scs)
    if alpha == 0.05:
        base_scs = scs
    print(f"    (took {time.time()-t0:.0f}s)")

print("\n=== finer sweep 0.05-0.15, all compared against the SAME alpha=0.05 baseline ===")
boost_at_05, _ = compute_boost_at(0.05)
base_scs_fixed = report("alpha=0.05 (baseline, ref)", build_pos(boost_at_05))
for alpha in (0.06, 0.07, 0.08, 0.09, 0.10, 0.12, 0.15):
    boost_at, avg_q = compute_boost_at(alpha)
    POS = build_pos(boost_at)
    report(f"alpha={alpha:<6} avgQ={avg_q:.1f}", POS, base_scs_fixed)
