"""
================================================================================
###  SAFE_llboost_v21.py  ·  v15 + G84 learned per-name rank-stability blend weight  ###
================================================================================
Built 2026-07-30, the third of three "extra edge" candidates investigated this session (resid
diversifier: rejected; E66 dense regime-gain sweep: rejected; this one: validated).

MECHANISM: v15's rank-stability blend uses ONE global RS_WEIGHT=0.015 applied uniformly to every
name, every day. This replaces that flat weight with a per-name multiplier learned from each name's
own trailing BOOST_IC_L(=250)-day causal IC of the RAW (pre-standardization) rank-stability signal
against its own realized idio return:
  mult_i(t) = clip(1 + G84_GAIN*ic_i(t), 0, G84_CAP)
  w_i(t) = RS_WEIGHT * mult_i(t)
A name where RS has recently, causally, actually been predictive gets an amplified blend weight; a
name where it hasn't gets shrunk toward/at zero. Before day max(BOOST_MIN_DAY, WARMUP+BOOST_IC_L)
(not enough IC history yet), falls back to the shipped uniform RS_WEIGHT exactly -- identical to
v15 in that regime.

Ported from test_batch100_G84.py's original coarse sweep (GAIN in {1,2,3,5} cap=2.0 fixed; CAP in
{1.5,2,3} gain=3.0 fixed -- missed the bar by 1.3 on NEW) via a denser 6x4 grid re-test against the
CURRENT champion (test_g84_learned_rs_dense.py). GAIN=7.0 for gain in the dense re-sweep, GAIN=2.0
CAP in {1.5,2,2.5,3.0} (cap doesn't bind at this gain, all four identical) is the clean peak:
real prices.txt OLD=885.8 (tied) NEW=916.9 (+3.1) rmean=918.1 (+0.8) rfloor=720.7 (tied)
n_worse=0/61 -- not a single one of the 61 rolling 250-day windows got worse. The gain-sweep shape
(1.0 -> 2.0 -> 3.0 -> 4.0+) is a smooth rise-then-fall local optimum, not an isolated spike.

VALIDATION (see test_v21_g84_dense.py): real data (now also checked against the 250-500 window, not
just OLD=500-750/NEW=750-1000), change-point reverse/rotate, trend-regime momentum/flip/noise --
same convention as test_v15_insurance_only.py / test_v19_twohop.py.
================================================================================
"""
import numpy as np
from scipy import stats

BOOK = "SAFE · LL-BOOST v21 (v15 + G84 learned per-name RS blend weight, gain=2.0 cap=2.0)"
