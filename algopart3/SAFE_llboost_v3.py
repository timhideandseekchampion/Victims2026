"""
================================================================================
###  SAFE_llboost_v3.py  ·  SAFE_llboost + volatility-restricted candidate    ###
###                          leader pool for the pairwise boost              ###
================================================================================
ONE change from SAFE_llboost.py: the boost's leader search is restricted to the
BOOST_N_CANDIDATES=39 highest (causally, trailing-realized-vol-ranked) volatility idio stocks,
instead of searching all 49. The Bonferroni divisor is adjusted to match (39, not 49).

Rationale (test_ncandidates_causal.py, part of a 20-idea test queue): the 10 lowest-vol idio
stocks make noisy, low-power "leader" candidates -- their small, quiet moves rarely carry a
real, tradeable lead-lag relationship, so including them in the leader search mostly just adds
false-discovery risk (a "significant" correlation on a low-vol series is more likely to be
noise that happens to clear the Bonferroni bar on a given day). Restricting the search pool
excludes exactly this fixed set of 10 names.

Validated (test_ncandidates_causal.py): swept N from 20 to 48 with a CAUSAL trailing vol ranking
(verified the causal and full-sample rankings are functionally identical here -- each stock's
volatility rank is a stable, persistent characteristic in this dataset, confirmed via 39/39
overlap between expanding-window and trailing-500-day rankings checked at days 600/800/999, so
this is not a look-ahead-sensitive choice). Found a genuine, monotonically-improving region from
N=29 up through N=39 (n_worse falling from 26/61 to 1/61), not an isolated spike -- unlike the
IC_EW_W=150 trap rejected earlier this session. N=39's single "worse" window (end-day 520) is
-0.1 points, essentially a rounding-level blip; every other window from day 500-1000 is flat or
a real gain (up to +52 points in the 850-990 stretch) -- a cleaner profile than SAFE_llboost_v2's
own 18/61-worse MOM_LB finding.
...
CAVEAT: N=40 (n_worse=21/61) sits right after N=39 with a sharp discontinuity -- the choice of
exactly 39 (rather than, say, 35-38, which are also solidly good at n_worse 7-16/61) is still a
single point picked from a generally-good but not perfectly smooth region. Treat as validated-but-
not-immune-to-some-parameter-sensitivity, in the same spirit as v2's own caveat.
================================================================================
...
BOOK = "SAFE · LL-BOOST (llvol idio + significance-gated pairwise boost, min-history gated)"
