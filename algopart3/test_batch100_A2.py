...
IDEA A2: Resweep the HALF_LIVES set composition against v10 -- try swapping/adding one of
100, 750, 1500, 3000 into the current (250, 500, 1000, 2000) set.

HALF_LIVES only affects the per-half-life ridge ensemble loop (the average of fs across half-lives).
Everything else (BLEND reversal leg, pairwise boost, rank-stability blend, ALGO leg) is independent
and precomputed ONCE, reused for every candidate half-life set.
"""
...
print("=== precompute: REV leg, pairwise boost, rank-stability signal, ALGO leg -- all independent "
      "of HALF_LIVES, cached once ===", flush=True)
t0 = time.time()
...
print(f"\n=== sanity check: HALF_LIVES={SHIPPED_HL} (shipped) must reproduce SAFE_llboost_v10 exactly ===")
t0 = time.time()
POS_base = build_pos(SHIPPED_HL)
...
def evaluate(nm, half_lives, verbose=True):
    Pz = build_pos(half_lives); scs = scs_curve(Pz)
    wo = wscore(Pz, *OLD); wn = wscore(Pz, *NEW)
    passed = (wo > base_wo) and (wn > base_wn) and (scs.mean() > base_scs.mean())
    nworse = int((scs < base_scs).sum())
    if verbose:
        tag = "  <== PASS" if passed else ""
        print(f"  {nm:<28}OLD={wo:7.1f}  NEW={wn:7.1f}  rmean={scs.mean():7.1f}  "
              f"rfloor={scs.min():7.1f}  n_worse={nworse}/{len(scs)}{tag}")
    return dict(name=nm, hl=half_lives, wo=wo, wn=wn, rm=scs.mean(), rf=scs.min(), nworse=nworse, passed=passed)


print(f"\n=== SWEEP: swap/add candidates into shipped {SHIPPED_HL} ===")
CANDIDATES = {
    "swap 250->100":  (100, 500, 1000, 2000),
    "swap 500->750":  (250, 750, 1000, 2000),
    "swap 1000->1500": (250, 500, 1500, 2000),
    "swap 2000->3000": (250, 500, 1000, 3000),
    "add 100":        (100, 250, 500, 1000, 2000),
    "add 750":        (250, 500, 750, 1000, 2000),
    "add 1500":       (250, 500, 1000, 1500, 2000),
    "add 3000":       (250, 500, 1000, 2000, 3000),
}
t0 = time.time()
results = [evaluate(nm, hl) for nm, hl in CANDIDATES.items()]
print(f"  sweep done ({time.time()-t0:.1f}s)")

passing = [c for c in results if c["passed"]]
print(f"\n{len(passing)}/{len(results)} HALF_LIVES variants beat v10 on OLD+NEW+rmean jointly.")
if passing:
    for c in passing:
        print(f"  {c['name']:<20} hl={c['hl']} rmean={c['rm']:.1f} n_worse={c['nworse']}/61")
best = max(results, key=lambda c: c["rm"])
print(f"Best by rolling mean: {best['name']} hl={best['hl']} (rmean={best['rm']:.1f} vs shipped "
