"""Step 2 (plan: revisit ALGO leg): re-sweep the ALGO leg's core parameters (VOL_WIN, VOL_Z,
IC_FAST, SWITCH_GAIN, IC_EW_HL, MOM_LB) against the CURRENT boosted idio book, not the older
pre-boost idio book they were originally validated against. Reuses the exact significance-gated
boost + ridge precompute pattern from test_boost_subparam_sweep.py for the (fixed) idio side, and a
faithful parameterized copy of SAFE_llvol.py's _algo_vol_shares for the ALGO leg being swept.
"""
...
def algo_vol_shares(lpA, cur0, cap_dol, VOL_WIN, VOL_Z, IC_FAST, SWITCH_GAIN, IC_EW_HL, IC_EW_W, MOM_LB, COMBINE_GAIN,
                     VOL_COMBINE=True):
    T_ = len(lpA)
...
DEFAULTS = dict(VOL_WIN=20, VOL_Z=60, IC_FAST=90, SWITCH_GAIN=2.5, IC_EW_HL=(20, 45), IC_EW_W=200,
                MOM_LB=10, COMBINE_GAIN=3.5)
...
def build_algo(**overrides):
    ...
    for k in range(130, nt):
        ...
        algo_pos[k] = np.clip(algo_vol_shares(logp[0, :k + 1], cur0, dlr[0], **params), -lim0, lim0)
    return algo_pos
...
base_scs = report("shipped defaults", full_pos(build_algo()))
...
for v in (5, 7, 10, 14, 20):
    report(f"MOM_LB={v}", full_pos(build_algo(MOM_LB=v)), base_scs)

print("\n=== sweep IC_EW_HL ===")
for v in [(15, 30), (20, 45), (20, 60), (30, 60), (15, 45)]:
    report(f"IC_EW_HL={v}", full_pos(build_algo(IC_EW_HL=v)), base_scs)

print("\n=== VOL_COMBINE: vol+momentum (current, LLVOL) vs vol-only (LLVOL_VO's mechanism) ===")
report("VOL_COMBINE=True (current)", full_pos(build_algo(VOL_COMBINE=True)), base_scs)
report("VOL_COMBINE=False (vol-only, LLVOL_VO)", full_pos(build_algo(VOL_COMBINE=False, SWITCH_GAIN=2.5)), base_scs)
for sg in (1.5, 2.0, 2.5, 3.0, 3.5, 4.0):
    report(f"vol-only SWITCH_GAIN={sg}", full_pos(build_algo(VOL_COMBINE=False, SWITCH_GAIN=sg)), base_scs)
