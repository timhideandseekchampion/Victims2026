"""
test_v32_fallback_blend.py -- validates SAFE_llboost_v32.py (v22 with the insurance layer's
winner-take-all fallback pick replaced by a weighted blend of ALL qualifying fallbacks) against:
  1. real data -- MUST be byte-identical to v22 (insurance layer never engages on real data
     regardless of pick-vs-blend, since champ is never "sick" there -- a strong structural check).
  2. change-point reverse mode -- the scenario where fallback usage was already CONFIRMED genuine
     (test_v14_reverse_mechanism.py's counterfactual, +$22,970 mean vs flatten-only).
  3. trend-regime momentum -- the scenario with heaviest fallback engagement (~138/150 days).

Run: python3 test_v32_fallback_blend.py
"""
import numpy as np, pandas as pd
import SAFE_llboost_v10 as V10
import SAFE_llboost_v22 as V22
import SAFE_llboost_v32 as V32
...
def real_data_check():
    print("=== 1. real prices.txt: v32 (blend) vs v22 (winner-take-all) -- expect byte-identical ===")
    ...
    POS22 = build(V22); POS32 = build(V32)
    mism = int((~np.all(POS22 == POS32, axis=0)).sum())
    print(f"  days with any position differing: {mism}/{nt-96} (expect 0)\n")
...
def run_changepoint(mode, seed):
    ...
    reset(V10); POS10 = walk_pos_idio(V10, out, NT_PRE)
    reset(V22); POS22 = walk_pos_idio(V22, out, NT_PRE)
    reset(V32); POS32 = walk_pos_idio(V32, out, NT_PRE)
    ...
    pnl32 = daily_pnl_idio(POS32, out, NT_PRE + 1, nt - 1)
    pnl_oracle = daily_pnl_idio(oracle_POS_full, out, NT_PRE + 1, nt - 1)

    cum10 = np.cumsum(pnl10)[-1]; cum22 = np.cumsum(pnl22)[-1]; cum32 = np.cumsum(pnl32)[-1]
    cum_oracle = np.cumsum(pnl_oracle)[-1]
    loss10 = cum_oracle - cum10
    saved22 = (loss10 - (cum_oracle - cum22)) / max(1.0, loss10) * 100
    saved32 = (loss10 - (cum_oracle - cum32)) / max(1.0, loss10) * 100
    mism = int((~np.all(POS22 == POS32, axis=0)).sum())
    print(f"  mode={mode:8s} seed={seed}: v10={cum10:9.0f}  v22={cum22:9.0f}  v32={cum32:9.0f}  "
          f"oracle={cum_oracle:9.0f}  saved22={saved22:5.1f}%  saved32={saved32:5.1f}%  "
          f"days v22!=v32={mism}")
    return saved32


def trend_regime():
    print("\n=== 3. trend-regime momentum/flip/noise (v32 vs v10/v22) ===")
    ...
    for kind in ("momentum", "flip", "noise"):
        ...
        reset(V10); POS10 = walk(V10, full, S, E)
        reset(V22); POS22 = walk(V22, full, S, E)
        reset(V32); POS32 = walk(V32, full, S, E)
        pnl10 = daily_pnl(POS10, full, S, E)
        pnl22 = daily_pnl(POS22, full, S, E)
        pnl32 = daily_pnl(POS32, full, S, E)
        print(f"  {kind:9s}: v10={pnl10.sum():9.0f}  v22={pnl22.sum():9.0f}  v32={pnl32.sum():9.0f}")


if __name__ == "__main__":
    real_data_check()

    print("=== 2. change-point experiment: v32 vs v22 (vs recorded v22 numbers) ===")
    results = {"reverse": [], "rotate": []}
    for mode in ("reverse", "rotate"):
        for seed in SEEDS:
            results[mode].append(run_changepoint(mode, seed))
    print("\n=== summary: v32 frac of oracle-gap recovered vs plain v10 ===")
    for mode in ("reverse", "rotate"):
        vals = results[mode]
        print(f"{mode}: v32 frac_saved per seed = {[round(v,1) for v in vals]}  mean={np.mean(vals):.1f}%")
