...
def _pick_at(a):
    ...
    SAFE_llboost_v14.py's _pick_at. Kept for reference/comparison; getMyPosition below uses the
    blended variant."""
    lo = a - ROT_W + 1
    if lo < WARMUP:
        return "champ"
...
def _choose(T):
    ...
    switching away from champ. Identical to SAFE_llboost_v14.py's _choose. Kept for reference."""
...
def _pick_at_blend(a):
    """Like _pick_at, but returns {name: weight} (normalized to sum 1) over EVERY qualifying
    fallback (both profitable and beating champ), instead of picking a single winner. None means
    champ is healthy or no fallback qualifies."""
    lo = a - ROT_W + 1
    if lo < WARMUP:
        return None
    pn_c = _pn("champ", lo, a + 1)
    ...
    for name in FALLBACKS:
        pf = _pn(name, lo, a + 1)
        if (pf - pn_c).sum() > 0.0 and pf.sum() > 0.0:
            weights[name] = pf.sum()
    if not weights:
        return None
    tot = sum(weights.values())
    return {name: w / tot for name, w in weights.items()}


def _choose_blend(T):
    """Same ROT_P-day persistence requirement as _choose (champ must be sick with at least one
    qualifying fallback on EVERY one of the trailing ROT_P days), but blends ALL qualifying
    fallbacks -- weighted by relative trailing strength -- using today's (freshest) weights, rather
    than requiring the identical single winner to persist across the whole window."""
    picks = [_pick_at_blend(a) for a in range(T - ROT_P, T)]
    if picks and all(p is not None for p in picks):
        return picks[-1]
    return None


def _sig_blend(weights, n):
    sig = np.zeros_like(_FB[n][FALLBACKS[0]])
    for name, w in weights.items():
        sig = sig + w * _FB[n][name]
    return sig


def _pn1_blend(weights, n):
    return float((np.sign(_sig_blend(weights, n)) * _RET[n]).sum())


def _pn_blend(weights, lo, hi):
    return np.array([_pn1_blend(weights, n) for n in range(lo, hi)])
...
def _kill(T, chosen):
    ...
    SAFE_llboost_v14.py's _kill. Kept for reference (chosen="champ" case)."""
    if not KILL_ON:
        return False
...
def _kill_blend(T, weights):
    """Same as _kill, but evaluates the trailing PnL-sum of the BLENDED (not single-name) signal."""
    if not KILL_ON:
        return False
    for a in range(T - KILL_P, T):
        lo = a - ROT_W + 1
...
def getMyPosition(prcSoFar):
    ...
    blend = _choose_blend(t) if ready else None

    if blend is None:
        wz = _SIG[t]
        killed = ready and _kill(t, "champ")
    else:
        wz = _sig_blend(blend, t)
        killed = ready and _kill_blend(t, blend)
