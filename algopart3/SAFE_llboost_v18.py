"""
================================================================================
###  SAFE_llboost_v18.py  ·  v17's dual EW IC gate, but Bonferroni-corrected for testing 2 hypotheses ###
================================================================================
Built 2026-07-29, in direct response to v17's own result: testing a short (15d) AND long (60d)
half-life IC t-stat and accepting the switch if EITHER clears IC_TCRIT=2.0 turned out to be a classic
multiple-comparisons problem -- passing one of two bars is easier than passing one bar alone, so v17
came out MORE PERMISSIVE across the board, not smarter. It matched v15's un-gated numbers in
momentum/flip (fine) but was WORSE than v16's single-test gate in the noise regime (-27,230 vs
v16's -25,760) -- exactly where a significance gate is supposed to help -- and did not actually
recover the reverse-mode edge it was built to fix (25.3% vs v16's 25.5%, worse if anything). The
short half-life's recency-weighting benefit was also offset by a smaller effective sample size
feeding the standard error, so "faster half-life" didn't even clearly speed up detection on its own.

FIX in this file: keep v17's dual-timescale OR structure (still test short AND long, still accept if
EITHER passes -- the idea itself, catching a fast-forming OR a persistent relationship, is sound) but
correct the per-test critical value for the fact that TWO simultaneous hypotheses are being tested,
exactly the way this file's OWN `_pairwise_boost`/`_sig_threshold` already Bonferroni-corrects its own
39-candidate simultaneous significance test. Previously it was v16/v17's own single flat/fixed
IC_TCRIT=2.0, un-corrected for the fact that v17 tests two thresholds, not one -- inflating the
overall false-positive rate roughly in proportion to how many "shots on goal" a noise-driven fallback
gets. Built to fix that inflation directly, not by guessing a bigger fixed constant.

MECHANISM CHANGE from v17: `_ic_ew_tstat` is renamed `_ic_ew_stat` and now ALSO returns the
weighting's effective sample size (n_eff = (sum w)^2 / sum(w^2), same formula
`_pairwise_boost`/`_corrmat_weighted` already use for their own decayed-selection effective-N). A new
`_ic_tcrit(n_eff)` computes a per-test Bonferroni-corrected one-sided critical t-value via
`stats.t.ppf`, dividing the SAME `IC_ALPHA=0.05` this file's `BOOST_ALPHA` convention already uses by
`N_IC_TESTS=2` (matching `_sig_threshold`'s exact pattern: `alpha_adj = ALPHA / n_simultaneous_tests`,
then `stats.t.ppf(1 - alpha_adj/2, df=n_eff-1)`). Each half-life's own t-stat is compared against ITS
OWN critical value (computed from its own, generally smaller, n_eff for the short half-life) rather
than a single fixed IC_TCRIT=2.0 for both.

VALIDATION (`test_v18_bonferroni_ic.py`): same three-harness convention as v15/v16/v17 -- real data
(still silent?), change-point reverse/rotate (does the correction recover the reverse-mode edge v16/
v17 both gave up, while keeping v16's noise-regime protection this time?), trend-regime
momentum/flip/noise. If this still doesn't beat v15/v16, the honest conclusion is that a
dual-timescale OR-gate isn't the right shape for this problem regardless of correction, and this
specific axis should be closed rather than tried a fourth way.
================================================================================
"""
import numpy as np
from scipy import stats

BOOK = "SAFE · LL-BOOST v18 (v17 + Bonferroni-corrected IC critical value, N_IC_TESTS=2)"

# --- idio ridge + ALGO adaptive-vol leg (identical to SAFE_llboost_v9-v16.py) ---
...
# --- pairwise boost parameters (plain, no decayed-selection fallback -- identical to v12/v15/v16) ---
BOOST_K = 1.5
BOOST_MIN_DAY = 480
...
# --- post-jump fixed-size fade (identical to SAFE_llboost_v12/v13/v14/v15/v16.py) ---
FADE_W        = 40
FADE_K_SIGMA  = 2.0
FADE_EXTRA_W  = 0.06

# --- idio kill switch, PnL-sum trigger (UNCHANGED from v11-v16 -- not gated in this file either) ---
KILL_ON     = True
KILL_MARGIN = 0.0
KILL_P      = 1
ROT_W       = 60

# --- momentum/xsac insurance layer (identical to SAFE_llboost_v14-v16.py's Part B) ---
...
# --- NEW in v17: DUAL exponentially-weighted IC/t-stat significance gate (replaces v16's flat one) ---
IC_EW_HL_SHORT  = 15     # fast half-life -- catches a genuinely new, fast-forming relationship
IC_EW_HL_LONG   = 60     # slow half-life -- close to v16's flat window, catches persistent ones
IC_ALPHA        = 0.05   # base two-sided significance level, same convention as BOOST_ALPHA
N_IC_TESTS      = 2      # short + long half-life -- Bonferroni-divide IC_ALPHA across both
...
def _rank_stability_signal(logp):
    """Identical to SAFE_llboost_v10-v16._rank_stability_signal."""
    t = logp.shape[1]
...
def _algo_vol_shares(lpA, cur0, cap_dol):
    ...
    Identical to SAFE_llboost_v9-v16 (unaffected by anything else in this file -- this leg has its
...
def _pairwise_boost(rs):
    """Identical to SAFE_llboost_v8/v9/v10/v11/v12/v15/v16._pairwise_boost -- plain, undecayed,
...
def _idio_signal(prcSoFar):
    ...
    SAFE_llboost_v15/v16._idio_signal. Returns a 50-vector (idio names only, ALGO excluded)."""
...
def _fallback_signals(P):
    """Identical to SAFE_llboost_v14-v16.py's Part B: mom/momJT/residMom computed from a small
...
def _pn1(name, n):
    ...
    over the 50 idio names. Identical to SAFE_llboost_v14-v16's generalized _pn1."""
    key = (name, n)
    v = _PN.get(key)
    if v is None:
...
def _ic1(name, n):
    """Daily cross-sectional IC (Pearson correlation) between signal `name`'s forecast and the same
    day's realized idio return, across the 50 stocks. Identical to SAFE_llboost_v16's `_ic1`."""
    key = (name, n)
    v = _ICD.get(key)
    if v is None:
...
def _ic_ew_stat(name, lo, hi, hl):
    """Exponentially-weighted (half-life hl) mean/variance/effective-N and t-stat of signal `name`'s
    daily cross-sectional IC over [lo, hi) -- same weighting style as
    `_ewls_ridge`/`_corrmat_weighted` elsewhere in this file. Unlike a flat-window t-stat (v16), this
    gives more weight to RECENT days, so a genuinely new relationship doesn't have to out-vote 55+
    days of stale pre-change history before it registers as significant. Returns (t_stat, n_eff) --
    n_eff (NEW in v18) feeds `_ic_tcrit`'s Bonferroni correction below."""
    ic = _ic(name, lo, hi)
    n = len(ic)
    if n < 10:
        return 0.0, 0.0
    ...
    vw = (w * (ic - mw) ** 2).sum() / sw
    n_eff = sw ** 2 / (w ** 2).sum()
    t = float(mw / (np.sqrt(vw / n_eff) + 1e-18))
    return t, float(n_eff)


def _ic_tcrit(n_eff):
    """NEW in v18: Bonferroni-corrected one-sided critical t-value for testing N_IC_TESTS=2
    simultaneous half-lives, using THIS test's own effective sample size -- exactly
    `_sig_threshold`'s established pattern in this same file (`alpha_adj = ALPHA / n_tests`, then
    `stats.t.ppf(1 - alpha_adj/2, df=...)`), just applied to the IC gate instead of the boost's
    candidate selection. The short half-life's smaller n_eff naturally gets a higher (stricter)
    critical value than the long half-life's, reflecting its noisier estimate -- not a fixed
    IC_TCRIT=2.0 for both regardless of how much effective evidence each one actually has."""
    if n_eff < 3:
        return 1e18
    alpha_adj = IC_ALPHA / N_IC_TESTS
    df = max(1.0, n_eff - 1)
    return float(stats.t.ppf(1 - alpha_adj / 2, df=df))
...
def _pick_at(a):
    ...
    OR xsac flags a momentum regime) AND a fallback is beating it AND that fallback's own EW-weighted
    IC t-stat clears its OWN Bonferroni-corrected critical value (NEW in v18) at EITHER a short
    (15-day) or long (60-day) half-life -- same dual-timescale OR structure as v17, but each test now
    compares against a critical value corrected for testing 2 simultaneous hypotheses, instead of
    v17's single fixed IC_TCRIT=2.0 for both regardless of how many "shots on goal" that gives a
    noise-driven fallback."""
    lo = a - ROT_W + 1
    if lo < WARMUP:
        return "champ"
    ...
    for name in FALLBACKS:
        ...
        if (pf - pn_c).sum() > 0.0 and pf.sum() > 0.0 and pf.sum() > best_v:
            t_short, n_short = _ic_ew_stat(name, lo, a + 1, IC_EW_HL_SHORT)
            t_long, n_long = _ic_ew_stat(name, lo, a + 1, IC_EW_HL_LONG)
            pass_short = t_short > _ic_tcrit(n_short)
            pass_long = t_long > _ic_tcrit(n_long)
            if pass_short or pass_long:          # NEW: Bonferroni-corrected dual-timescale veto
...
def _choose(T):
    ...
    switching away from champ. Identical to SAFE_llboost_v14-v16's _choose."""
...
def _kill(T, chosen):
    """UNCHANGED from v15/v16 -- deliberately not gated in this file either. Flattens the idio book
    if whichever signal is currently chosen has its trailing-ROT_W summed realized PnL-sign proxy
    below KILL_MARGIN, re-evaluated fresh each day (KILL_P=1)."""
    if not KILL_ON:
        return False
