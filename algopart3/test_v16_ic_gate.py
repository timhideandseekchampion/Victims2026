"""
test_v16_ic_gate.py -- validates SAFE_llboost_v16 (v15 + IC/t-stat significance gate on the
...
def real_data_check():
    print("=== 1. real prices.txt: v16 vs v12 (v15 already confirmed identical to v12) ===")
    ...
    print(f"  v16: OLD={wscore(POS16,P_,500,750,nInst):.1f} NEW={wscore(POS16,P_,750,nt,nInst):.1f} "
...
def run_changepoint(mode, seed):
    ...
    reset_module_state(V16); POS16 = walk_pos_idio(V16, out, NT_PRE)
    ...
    print(f"  mode={mode:8s} seed={seed}: v10={cum10:9.0f}  v15={cum15:9.0f}  v16={cum16:9.0f}  "
...
def trend_regime():
    print("\n=== 3. trend-regime momentum/flip/noise (v16 vs v10/v15) ===")
    ...
    for kind in ("momentum", "flip", "noise"):
        ...
        reset_module_state(V16); POS16 = walk(V16, full, S, E)
        pnl10 = daily_pnl(POS10, full, S, E)
        pnl16 = daily_pnl(POS16, full, S, E)
        print(f"  {kind:9s}: v10={pnl10.sum():9.0f}  v16={pnl16.sum():9.0f}")


if __name__ == "__main__":
    real_data_check()

    print("=== 2. change-point experiment: v16 vs v15 (vs recorded v15 numbers) ===")
    results = {"reverse": [], "rotate": []}
    for mode in ("reverse", "rotate"):
        for seed in SEEDS:
            results[mode].append(run_changepoint(mode, seed))
    print("\n=== summary: v16 frac of oracle-gap recovered vs plain v10 ===")
    for mode in ("reverse", "rotate"):
        vals = results[mode]
        print(f"{mode}: v16 frac_saved per seed = {[round(v,1) for v in vals]}  mean={np.mean(vals):.1f}%")
