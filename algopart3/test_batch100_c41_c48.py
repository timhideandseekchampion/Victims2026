"""
test_batch100_c41_c48.py

Batch of 8 ideas (C41-C48), ALL modifying only the leader-detection / combination step inside
...
reused VERBATIM from V10 and is unaffected by any idea here -- computed once, shared across all 8.
  ...
  C44: cluster-restricted candidate pool -- k-means (k=5) on the causal idio correlation matrix;
       restrict each follower's leader search to its OWN cluster (no global top-39-by-vol filter).
  C45: distance correlation (not Pearson) for leader detection. Vectorized via double-centered
       ...
       threshold as a magnitude-matched cutoff (dCor's true null distribution differs from a
       correlation t-test -- flagged as an approximation appropriate to a screening pass).
  C46: graphical-lasso sparse inverse covariance (GraphicalLassoCV) replacing the top-39-by-vol pool
       + implicit "all pairs" search with a sparse neighbor graph; refit every 25 days (CV fit costs
       ~1s, refitting daily over ~520 days is not a "screening pass" cost) -- graph held fixed
       between refits. Given each follower's leader search, still uses the SAME lag-1 Pearson +
       significance-style downstream logic as v10, only the *candidate universe* changes.
  C47: Granger causality (statsmodels, maxlag=1) REPLACES the Pearson significance-threshold gate on
       the already-corr-selected best candidate (selection itself still by argmax |corr|, matching
       "replace the significance-thresholded correlation TEST", not the argmax selection) -- a full
       pairwise re-search via Granger over all 39 candidates x 50 followers x ~520 days would be
       ~20x the compute for the same conclusion and is out of scope for a screening pass.
  C48: use a 2-day trailing average of the (already-selected) leader's return, instead of the single
       most recent day, as the input to the sign/power transform -- applied consistently through the
       scale calc, the historical IC-gate series, and the final applied value.

All 8 reuse V10._ewls_ridge, V10._beta_adjusted_target, V10._algo_vol_shares,
V10._rank_stability_signal, V10._corrmat, V10._sig_threshold, and V10's constants directly. A new
`combine_leaders()` helper generalizes _pairwise_boost's per-follower scale/power-transform + IC>0
gate + apply-lead_boost[-1] body to an arbitrary list of (candidate, weight) pairs -- this is the
literal mechanism every one of C41/42/43/44/45 needs to change (single-best -> weighted-multi), so
it is NOT a from-scratch reimplementation, it's _pairwise_boost's own inner loop body lifted out and
parameterized.
"""
import numpy as np, pandas as pd, time, io, contextlib
...
print("=== precompute (shared across all 8 ideas): ridge WZ (beta-adjusted target) + BLEND reversion "
...
def build_pos(boost_by_day):
    """boost_by_day: dict/array-like t -> boost vector (nIdio,), only consulted for t>=BOOST_MIN_DAY."""
...
if not SANITY_OK:
    ...
    print("  OK -- matches v10 to within rounding. This sanity check underlies EVERY idea below "
          "(all 8 share this exact precompute; only the boost/leader-detection call site differs).")
...
# =========================================================================================
# C44: cluster-restricted candidate pool (k-means, k=5, on the causal all-pairs corr matrix).
# =========================================================================================
...
def boost_cluster_at_day(k, n_clusters=N_CLUSTERS_C44):
    rsl = rs[:, :k]; n, T = rsl.shape
    boost = np.zeros(n)
    Xi_full = rsl[:, :-1]; Yj = rsl[:, 1:]
    n_samples = Xi_full.shape[1]
...
# =========================================================================================
# C47: Granger causality (maxlag=1) replaces the Pearson significance-threshold GATE on the
# already-argmax-selected candidate (see docstring for why a full pairwise Granger re-search is
# out of scope). Windowed to the last GRANGER_WINDOW samples for tractability.
# =========================================================================================
...
def boost_granger_at_day(k):
    ...
    for j in range(n):
        ...
        ci = int(np.nanargmax(np.abs(col)))
        i = int(cand_idx[ci])
        a = max(0, n_samples - GRANGER_WINDOW)
        xw = Xi_full[cand_idx[ci], a:]; yw = Yj[j, a:]
        ok = ~np.isnan(xw) & ~np.isnan(yw)
        if ok.sum() < 60 or xw[ok].std() < 1e-12: continue
        p = granger_pvalue(xw[ok], yw[ok])
        if p >= alpha_adj: continue
        boost[j] = combine_leaders(rsl, j, T, [(i, 1.0)])
    return boost


# =========================================================================================
# C48: 2-day trailing average of the (unchanged, argmax-selected) leader's return as the boost
# input, applied consistently through scale calc + IC-gate history + final applied value.
# =========================================================================================
def boost_avg2_at_day(k):
    rsl = rs[:, :k]; n, T = rsl.shape
    boost = np.zeros(n)
    Xi_full = rsl[:, :-1]; Yj = rsl[:, 1:]
    n_samples = Xi_full.shape[1]
    ...
    for j in range(n):
        ...
        i = int(cand_idx[ci])
        lead_raw = rsl[i]
        lead = 0.5 * (lead_raw + np.concatenate(([lead_raw[0]], lead_raw[:-1])))
        scale = np.nanstd(lead[max(0, T - 1 - BOOST_SCALE_W):T - 1]) + 1e-12
        lead_boost = np.sign(lead) * (np.abs(lead) / scale) ** BOOST_P
        a = max(0, T - 1 - BOOST_IC_L)
        xs = lead_boost[a:T - 1]; ys = rsl[j, a + 1:T]
...
for K in (1, 2, 3):
    ...
    tag = "K=1 (must match baseline)" if K == 1 else f"K={K}"
    nm = f"topK={K}"
    if K == 1:
    ...
    res = evaluate(f"{nm} ({tag})" if K == 1 else nm, BOOST, base_wo, base_wn, base_scs)
...
print(f"\n=== C44: cluster-restricted candidate pool (k-means, k={N_CLUSTERS_C44}) ===", flush=True)
t0 = time.time()
BOOST44 = np.zeros((nIdio, nt))
for k in range(BOOST_MIN_DAY, nt):
    BOOST44[:, k] = boost_cluster_at_day(k)
results["cluster_k5"] = evaluate("cluster_k5", BOOST44, base_wo, base_wn, base_scs)
print(f"    [{time.time()-t0:.0f}s]", flush=True)
...
print(f"\n=== C47: Granger causality gate (maxlag=1, window={GRANGER_WINDOW}) ===", flush=True)
t0 = time.time()
BOOST47 = np.zeros((nIdio, nt))
for k in range(BOOST_MIN_DAY, nt):
    BOOST47[:, k] = boost_granger_at_day(k)
results["granger"] = evaluate("granger", BOOST47, base_wo, base_wn, base_scs)
print(f"    [{time.time()-t0:.0f}s]", flush=True)

print("\n=== C48: 2-day trailing average leader return as boost input ===", flush=True)
t0 = time.time()
BOOST48 = np.zeros((nIdio, nt))
for k in range(BOOST_MIN_DAY, nt):
    BOOST48[:, k] = boost_avg2_at_day(k)
results["avg2"] = evaluate("avg2", BOOST48, base_wo, base_wn, base_scs)
print(f"    [{time.time()-t0:.0f}s]", flush=True)
