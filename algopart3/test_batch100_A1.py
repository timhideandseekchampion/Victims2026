...
IDEA A1: Resweep RIDGE_A (shared ridge penalty across all 4 HALF_LIVES) against v10, e.g.
0.03, 0.05, 0.1, 0.15, 0.2, 0.3. Shipped value is 0.1.

RIDGE_A only affects the _ewls_ridge fit inside the per-half-life ensemble loop. Everything else
(BLEND reversal leg, pairwise boost, rank-stability blend, ALGO leg) is independent of RIDGE_A and
is precomputed ONCE and reused for every candidate value.
"""
...
print("=== precompute: REV leg, pairwise boost, rank-stability signal, ALGO leg -- all independent "
      "of RIDGE_A, cached once ===", flush=True)
t0 = time.time()
...
print(f"\n=== sanity check: RIDGE_A={SHIPPED_A} (shipped) must reproduce SAFE_llboost_v10 exactly ===")
t0 = time.time()
POS_base = build_pos(SHIPPED_A)
base_scs = scs_curve(POS_base)
base_wo, base_wn = wscore(POS_base, *OLD), wscore(POS_base, *NEW)
print(f"  RIDGE_A={SHIPPED_A}: OLD={base_wo:.1f}  NEW={base_wn:.1f}  rmean={base_scs.mean():.1f}  "
...
def evaluate(ridge_a, verbose=True):
    Pz = build_pos(ridge_a); scs = scs_curve(Pz)
    wo = wscore(Pz, *OLD); wn = wscore(Pz, *NEW)
    passed = (wo > base_wo) and (wn > base_wn) and (scs.mean() > base_scs.mean())
    nworse = int((scs < base_scs).sum())
    if verbose:
        tag = "  <== PASS" if passed else ""
        print(f"  RIDGE_A={ridge_a:<6}OLD={wo:7.1f}  NEW={wn:7.1f}  rmean={scs.mean():7.1f}  "
              f"rfloor={scs.min():7.1f}  n_worse={nworse}/{len(scs)}{tag}")
    return dict(a=ridge_a, wo=wo, wn=wn, rm=scs.mean(), rf=scs.min(), nworse=nworse, passed=passed)


print(f"\n=== SWEEP: RIDGE_A (shipped={SHIPPED_A}) ===")
SWEEP = [0.03, 0.05, 0.1, 0.15, 0.2, 0.3]
t0 = time.time()
results = [evaluate(a) for a in SWEEP]
print(f"  sweep done ({time.time()-t0:.1f}s)")

passing = [c for c in results if c["passed"]]
print(f"\n{len(passing)}/{len(results)} RIDGE_A values beat v10 on OLD+NEW+rmean jointly.")
best = max(results, key=lambda c: c["rm"])
print(f"Best by rolling mean: RIDGE_A={best['a']} (rmean={best['rm']:.1f} vs shipped rmean={base_scs.mean():.1f})")
