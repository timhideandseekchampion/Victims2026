"""
================================================================================
###  SAFE_llboost_v19.py  ·  v15 + two-hop transitive boost (batch100's C43, re-tested fresh)  ###
================================================================================
Built 2026-07-30, following up on a "closest legitimate near-miss" already on record in this
repo's README (the ~50/100-idea signal searches): C43, two-hop transitive boost, scored
NEW+1.7/rmean+0.3/rfloor unchanged/n_worse=0/61 against the-then-current-best (plain
SAFE_llboost_v10) -- missed the shipping bar only because OLD was an exact tie, not strictly
beaten. That number is STALE relative to what's actually being built on now: v10 has since been
superseded by v12's fade and v15's insurance layer, both computed on top of the SAME
`_pairwise_boost` this idea modifies. Per this repo's own standing rule (never trust a cited
number without re-deriving it against the CURRENT source), this file ports the idea fresh onto
v15 and re-validates from scratch -- see `test_v19_twohop.py` for real-data/change-point/
trend-regime numbers, not the old v10-era ones.

MECHANISM CHANGE from v15: `_pairwise_boost` tries each follower's existing single direct leader
first (identical selection to v15 -- argmax |corr| among the BOOST_N_CANDIDATES most-volatile
candidates, Bonferroni-significant). If that leader B itself has ITS OWN direct leader A (A != the
follower, A != B, and A is in the candidate pool) with a DIRECTLY significant |corr(A, follower)|,
A is added as a second candidate. The (at most two) candidates are combined via `_combine_leaders`,
a |corr|-weighted average of each candidate's OWN validated contribution (each still individually
gated on its own realized IC > 0 over the trailing BOOST_IC_L window) -- not a free pass for A, it
must independently clear the same bar B did. Whenever no valid two-hop chain exists for a follower,
this reduces exactly to v15's plain single-leader boost (verified in `test_v19_twohop.py`'s own
internal check). Everything else (ridge ensemble, beta-adjusted target, BLEND reversion,
rank-stability blend, post-jump fade, kill switch, momentum/xsac insurance layer) is unchanged from
v15, verbatim.
================================================================================
Built 2026-07-29 in response to this session's own v13/v14 validation results (see README's
"v13/v14 -- a mixed result, NOT a clean ship" section): v14 = v11's kill switch + v12's fade +
v13's gated boost fallback + a NEW momentum/xsac insurance layer, and validation found the two
additions bundled into v14 are NOT equally good --

  - v13's gated decayed-selection boost fallback: engages on 28/904 real days, and on real data
    scores n_worse=25/61 vs n_better=27/61 against v11 -- a COIN FLIP, the same rejected failure
    signature as the earlier BETA_DEMEAN_W=550 spike and the exponentially-decayed
    candidate-selection idea. NOT a clean win, despite attractive-looking OLD/NEW/rmean headline
    numbers. This file omits it entirely -- `_pairwise_boost` here is IDENTICAL to
    SAFE_llboost_v9/v10/v11/v12's plain, undecayed, full-history-only version.

  - v14's Part-B momentum/xsac insurance layer: silent on real data (validator picks `champ` every
    day, confirmed identical to v12 below), a genuine ~95%-of-upper-bound win in a synthetic
    momentum-regime trend-injection test, and a REAL (not spurious -- confirmed via a
    flatten-vs-trade-fallback counterfactual, `test_v14_reverse_mechanism.py`) improvement in the
    reverse-mode pairwise-break change-point scenario. This file KEEPS this layer, verbatim from
    SAFE_llboost_v14.py.

HYPOTHESIS THIS FILE TESTS: since v13's problem is specifically its own mechanism (not an
interaction with the insurance layer -- the two were shown to combine additively on real data in
v14), dropping v13 and keeping v12 + the insurance layer should keep the insurance layer's genuine
gains (momentum-regime win, reverse-mode edge) while being position-identical to v12 on real data
(no coin-flip risk, since v12 alone already has real-data n_worse=0/61 as a shipped mechanism).

MECHANISM: `_idio_signal` = v12's exact ridge ensemble + beta-adjusted target + BLEND reversion +
plain pairwise boost + rank-stability blend + post-jump fade. `_choose`/`_kill`/`_xsac` insurance
wrapper = v14's Part B, byte-identical (PnL-sum "champ sick" check, FALLBACKS=(mom, momJT,
residMom), ROT_P=5-day persistence, generalized 2-arg `_kill`).

VALIDATION (see `test_v15_insurance_only.py`):
  1. Real prices.txt: must be position-identical to SAFE_llboost_v12.py (validator silent, `_choose`
     returns "champ" every day) -- if true, this file inherits v12's own already-clean 0/61 real-data
     record for free, by construction, not empirically re-swept.
  2. Change-point reverse/rotate (same generator/seeds as test_v14_changepoint.py): does the
     reverse-mode gain survive without v13's contribution? (v13's own numbers were themselves
     already a wash in reverse mode, so it should.)
  3. Trend-regime momentum/flip/noise (test_v14_trend_regime.py's generator): does the momentum win
     and the flip/whipsaw cost reproduce without v13 in the mix?
================================================================================
"""
import numpy as np
from scipy import stats

BOOK = "SAFE · LL-BOOST v19 (v15 + two-hop transitive boost, re-tested C43)"
...
def _pairwise_boost(rs):
    """Two-hop transitive boost. Each follower j's direct leader B is selected exactly as in the
    plain single-leader boost (argmax |corr| among the BOOST_N_CANDIDATES most-volatile
    candidates, Bonferroni-significant). If B itself has its own direct leader A (A != j, A != B,
    A in the candidate pool) with a DIRECTLY significant |corr(A, j)|, A is added as a second
    candidate. The (at most two) candidates are combined via `_combine_leaders`, a |corr|-weighted
    average, each independently gated on its own realized validation IC > 0 -- A gets no free pass
    for being B's leader, it must clear the same bar directly against j. Reduces exactly to the
    plain single-leader boost whenever no valid two-hop chain exists."""
    n, T = rs.shape
    boost = np.zeros(n)
    if T < BOOST_MIN_DAY:
        return boost
...
def _idio_signal(prcSoFar):
    ...
    reversion, plain pairwise boost (v12, NOT v13's gated fallback), rank-stability blend,
    post-jump fixed-size fade (v12, applied last). Returns a 50-vector (idio names only, ALGO
    excluded)."""
