"""Item 3 [SIGNAL/MECH]: quantile-regression-based forecast (median target, sign-traded) instead
of ridge's mean-squared-error target. Refit periodically (every REFIT_FREQ days) per idio target
on a trailing window, for tractability -- 50 separate QuantileRegressor fits per refit point.
"""
...
for t in range(SAFE.WARMUP, nt):
    ...
    WZ_SHIP[t] = wz
base_scs = report("shipped ridge (baseline)", combine_and_score(WZ_SHIP))

print("\n=== item 3: quantile regression (median), periodic refit, trailing window ===")
REFIT_FREQ = 20
TRAIN_W = 500


def build_wz_quantreg(alpha_l1=0.001, quantile=0.5):
    WZ = {}
    coefs = None
    intercepts = None
    last_refit = -10_000
    ...
    for t in range(SAFE.WARMUP, nt):
        if t - last_refit >= REFIT_FREQ or coefs is None:
            ...
            Xtr = r[:, a:t - 1].T
            Ytr = rs[:, a + 1:t].T
            coefs = np.zeros((ntgt, nInst)); intercepts = np.zeros(ntgt)
            for j in range(ntgt):
                y = Ytr[:, j]
...
t0 = time.time()
WZQ = build_wz_quantreg()
print(f"  quantreg fit done ({time.time()-t0:.0f}s)")
report("quantile regression (median)", combine_and_score(WZQ), base_scs)
