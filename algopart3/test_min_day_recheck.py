"""Close the loop: BOOST_MIN_DAY's neighbor-stability was validated against the ORIGINAL
scale_w=500/IC_L=220, before the sub-parameter tune (scale_w=1000, IC_L=190). Re-check it holds
...
print("=== precompute (fixed): shipped WZ, ALGO leg ===")
WZ = {}
...
def build_pos(min_day, K=BOOST_K):
    boost_at = compute_boost_at(min_day)
...
print("\n=== baseline: min_day=500 (current shipped, scale_w=1000/IC_L=190) ===")
base_scs = report("min_day=500 (shipped)", build_pos(500))

print("\n=== neighbor-stability check ===")
for md in (400, 450, 480, 520, 550, 600):
    report(f"min_day={md}", build_pos(md), base_scs)
