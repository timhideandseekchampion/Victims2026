"""
test_batch100_A21.py

A21: Resweep BOOST_ALPHA against v10 (0.01, 0.03, 0.05(shipped), 0.08, 0.10), everything else (idio
ridge w/ beta-adjusted target, BLEND reversion, ALGO leg, rank-stability blend) held fixed at v10.

BOOST_ALPHA lives ONLY inside V10._sig_threshold (used by V10._pairwise_boost), a module-level global
read at call time -- so each candidate value is applied by monkeypatching V10.BOOST_ALPHA immediately
before calling V10._pairwise_boost verbatim, then restoring it (no reimplementation of the boost
mechanism). The expensive part (ridge ensemble w/ beta-adjusted target, BLEND reversion, ALGO leg,
rank-stability signal) is independent of BOOST_ALPHA and is computed ONCE and reused for every config,
exactly like test_v19cand_boost_ncandidates.py's caching pattern (same pattern also used by the
existing test_batch100_boostsweeps.py for A17-A21 jointly; this script isolates just A21 to avoid
recomputing the other 4 boost sub-params' sweeps).
"""
...
print("=== precompute (shared, independent of BOOST_ALPHA): ridge WZ w/ beta-adjusted target + BLEND "
      "reversion + ALGO leg + rank-stability signal, verbatim from v10 ===", flush=True)
t0 = time.time()
...
def compute_boost(**overrides):
    ...
    verbatim, only BOOST_ALPHA changes."""
...
print("\n=== sanity check: default BOOST_ALPHA must reproduce SAFE_llboost_v10 exactly ===")
t0 = time.time()
...
passing = [c for c in r21 if c["passed"]]
best = max(r21, key=lambda c: c["rm"])
print(f"\n{len(passing)}/{len(r21)} BOOST_ALPHA values beat v10 on OLD+NEW+rmean jointly. "
      f"Best by rmean: {best['name']} (rmean={best['rm']:.1f} vs shipped {base_scs.mean():.1f})")
