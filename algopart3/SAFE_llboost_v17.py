"""
================================================================================
###  SAFE_llboost_v17.py  ·  v15 + a DUAL exponentially-weighted IC significance gate  ###
================================================================================
Built 2026-07-29, in direct response to v16's own result: a single FLAT 60-day IC/t-stat
significance gate on the fallback switch correctly blocked every switch in the "nothing real is
happening" scenarios (bootstrap, noise -- byte-identical to v12 there) but COST real signal in every
scenario where a genuine change actually happened (reverse-mode edge dropped from v15's 28.2% to
25.5%; momentum-regime capture also slipped slightly) -- and did NOT fix the one thing that mattered
most for worst-case drawdown (the flip/whipsaw scenario's maxDD was identical to v15's, to the
dollar).

DIAGNOSIS (from the v16 write-up): a flat 60-day window gives a day from 59 days ago the SAME weight
as yesterday -- so right after a genuine, fast-forming regime shift, the new relationship's true
signal is diluted by ~55+ days of pre-change (irrelevant) history, and the t-stat takes too long to
climb above IC_TCRIT even though the underlying relationship is already real and already paying off
in raw PnL. This is EXACTLY the same problem the ridge ensemble itself solves for its own forecast
via `HALF_LIVES=(250,500,1000,2000)` half-life blending, and that the ALGO leg's own `_side` function
already solves for ITS significance check via `IC_EW_HL=(20,45)` (a fast+slow exponentially-weighted
IC pair) -- this file applies that SAME already-validated pattern to the fallback-switch's
significance test, which v16 never used.

MECHANISM CHANGE from v16: `_ic1`/`_ic` (the raw per-day cross-sectional IC array) are unchanged, but
the single flat-window `_tstat` check is replaced by `_ic_ew_tstat`, computing an EXPONENTIALLY
WEIGHTED mean/variance/effective-N (same weighting style as `_ewls_ridge`/`_corrmat_weighted`
elsewhere in this file) at TWO half-lives:
  IC_EW_HL_SHORT = 15   fast -- reacts within days of a genuine new relationship forming
  IC_EW_HL_LONG  = 60   slow -- close to v16's original flat window, catches persistent relationships
A fallback passes the significance gate if EITHER weighted t-stat exceeds IC_TCRIT=2.0 (an OR, not an
AND) -- deliberately permissive on which timescale confirms it, so a fast-forming genuine signal
doesn't have to wait for the slow window to catch up, while a purely noise-driven "win" still has to
clear a real bar on at least one timescale (unlike v15's no-bar-at-all). This combination rule is a
first pass, NOT swept against an AND-of-both or a single-EW-window alternative -- stated plainly, per
this repo's own convention for untuned choices.

VALIDATION (`test_v17_dual_ew_ic.py`): same three-harness convention as v15/v16 -- real data (still
silent?), change-point reverse/rotate (does the short half-life recover more of the reverse-mode edge
v16 gave up, without losing the bootstrap/noise protection?), trend-regime momentum/flip/noise.
================================================================================
"""
import numpy as np
from scipy import stats

BOOK = "SAFE · LL-BOOST v17 (v15 + dual exponentially-weighted IC significance gate, HL=15/60)"
...
IC_TCRIT        = 2.0   # min weighted t-stat, either timescale, to accept the switch
...
def _ic_ew_tstat(name, lo, hi, hl):
    """NEW in v17: exponentially-weighted (half-life hl) mean/variance/effective-N and t-stat of
    signal `name`'s daily cross-sectional IC over [lo, hi) -- same weighting style as
    ...
    days of stale pre-change history before it registers as significant."""
    ic = _ic(name, lo, hi)
    n = len(ic)
    if n < 10:
        return 0.0
    ...
    vw = (w * (ic - mw) ** 2).sum() / sw
    n_eff = sw ** 2 / (w ** 2).sum()
    return float(mw / (np.sqrt(vw / n_eff) + 1e-18))
...
def _pick_at(a):
    ...
    OR xsac flags a momentum regime) AND a fallback is beating it AND (NEW in v17) that fallback's
    own EW-weighted IC t-stat exceeds IC_TCRIT at EITHER a short (15-day) or long (60-day) half-life
    -- replaces v16's single flat-window significance test, deliberately more permissive on WHICH
    timescale confirms it so a fast-forming genuine relationship isn't blocked while it accumulates
    enough flat-window evidence."""
    lo = a - ROT_W + 1
    if lo < WARMUP:
        return "champ"
    ...
    for name in FALLBACKS:
        ...
        if (pf - pn_c).sum() > 0.0 and pf.sum() > 0.0 and pf.sum() > best_v:
            t_short = _ic_ew_tstat(name, lo, a + 1, IC_EW_HL_SHORT)
            t_long = _ic_ew_tstat(name, lo, a + 1, IC_EW_HL_LONG)
            if t_short > IC_TCRIT or t_long > IC_TCRIT:      # NEW: dual-timescale significance veto
