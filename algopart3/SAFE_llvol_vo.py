...
Backtest vs the alternatives (exact eval score, idio book identical throughout):
  ...
  SAFE_llvol (THIS)        685            755            754            551   (+ fast/slow-agree IC blend)
The graded window (751-1000) went 674 -> 721 (gain tuning, validated in BOTH disjoint sub-periods)
-> 755 (recency-blend IC). Live eval: mean(PL) 772.6, annSharpe 6.47, Score 754.54. The blend gives
up a little rolling floor (597->551) but ONLY in the ancient day-190-440 window finals won't revisit;
recent windows get stronger. Idio book (instruments 1..49) is byte-identical to SAFE.py throughout.
...
BOOK      = "SAFE · LL-VOL·VO (VOL-ONLY index leg — no momentum leg)"
...
VOL_COMBINE = False     # VOL-ONLY variant: the momentum leg is OFF, so the ALGO leg is purely the adaptive
                        # realized-vol switch (sized by SWITCH_GAIN=2.5). This is the sibling of SAFE_llvol.py
                        # (which has VOL_COMBINE=True) — kept as a separate book so the dashboards can show
                        # vol-only vs vol+momentum side by side. vol-only: rolling mean 740 / floor 550.
MOM_LB      = 10        # momentum lookback (days) for the z-scored return leg
COMBINE_GAIN = 3.0      # size on the summed (vol + momentum) adaptive-switch signals. Tuned 1.05->3.0 and
