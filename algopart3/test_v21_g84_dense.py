"""
test_v21_g84_dense.py -- validates SAFE_llboost_v21 (v15 + G84 learned per-name RS blend weight,
gain=2.0 cap=2.0) against real data, change-point reverse/rotate, and trend-regime
momentum/flip/noise -- same convention as test_v15_insurance_only.py / test_v19_twohop.py, PLUS a
new WIN250 (day 250-500) real-data window in addition to OLD (500-750) and NEW (750-1000), per
user request: don't only check the two windows the model is already mature for, also check the
earlier, less-mature window.

Recorded v15 baselines being compared against (this session):
  real data:      OLD=885.8 NEW=913.8 rmean=917.3 rfloor=720.7, n_worse=0/61 vs v12
...
Run: python3 test_v21_g84_dense.py
"""
...
def real_data_check():
    print("=== 1. real prices.txt: v21 vs v15 (now incl. WIN250=250-500, not just OLD/NEW) ===")
    ...
    curve21 = np.array([wscore(POS21, P_, E - NUMTEST, E, nInst) for E in end_days])
    n_worse = int((curve21 < curve15).sum()); n_better = int((curve21 > curve15).sum())
    mism = int((~np.all(POS15 == POS21, axis=0)).sum())

    win250_15 = wscore(POS15, P_, 250, 500, nInst); win250_21 = wscore(POS21, P_, 250, 500, nInst)
    old15 = wscore(POS15, P_, 500, 750, nInst); old21 = wscore(POS21, P_, 500, 750, nInst)
    new15 = wscore(POS15, P_, 750, nt, nInst); new21 = wscore(POS21, P_, 750, nt, nInst)

    print(f"  v15: WIN250={win250_15:.1f}  OLD={old15:.1f}  NEW={new15:.1f}  "
          f"rmean={curve15.mean():.1f}  rfloor={curve15.min():.1f}")
    print(f"  v21: WIN250={win250_21:.1f}  OLD={old21:.1f}  NEW={new21:.1f}  "
          f"rmean={curve21.mean():.1f}  rfloor={curve21.min():.1f}")
    print(f"  days with any position differing: {mism}/{nt-96}  n_worse={n_worse}/61  n_better={n_better}/61")
    passed = (win250_21 >= win250_15) and (old21 > old15) and (new21 > new15) and (curve21.mean() > curve15.mean())
    print(f"  joint pass (WIN250 not worse AND OLD/NEW/rmean all better): {passed}\n")
...
def run_changepoint(mode, seed):
    ...
    reset_module_state(V21); POS21 = walk_pos_idio(V21, out, NT_PRE)
    ...
    pnl21 = daily_pnl_idio(POS21, out, NT_PRE + 1, nt - 1)
    pnl_oracle = daily_pnl_idio(oracle_POS_full, out, NT_PRE + 1, nt - 1)

    cum10 = np.cumsum(pnl10)[-1]; cum15 = np.cumsum(pnl15)[-1]; cum21 = np.cumsum(pnl21)[-1]
    cum_oracle = np.cumsum(pnl_oracle)[-1]
    loss10 = cum_oracle - cum10; loss15 = cum_oracle - cum15; loss21 = cum_oracle - cum21
    saved15 = (loss10 - loss15) / max(1.0, loss10) * 100
    saved21 = (loss10 - loss21) / max(1.0, loss10) * 100
    mism = int((~np.all(POS15 == POS21, axis=0)).sum())
    print(f"  mode={mode:8s} seed={seed}: v10={cum10:9.0f}  v15={cum15:9.0f}  v21={cum21:9.0f}  "
          f"oracle={cum_oracle:9.0f}  saved15={saved15:5.1f}%  saved21={saved21:5.1f}%  "
          f"days v15!=v21={mism}")
    return saved21


def trend_regime():
    print("\n=== 3. trend-regime momentum/flip/noise (v21 vs v10/v15) ===")
    ...
    for kind in ("momentum", "flip", "noise"):
        ...
        reset_module_state(V21); POS21 = walk(V21, full, S, E)
        pnl10 = daily_pnl(POS10, full, S, E)
        pnl21 = daily_pnl(POS21, full, S, E)
        print(f"  {kind:9s}: v10={pnl10.sum():9.0f}  v21={pnl21.sum():9.0f}")


if __name__ == "__main__":
    real_data_check()

    print("=== 2. change-point experiment: v21 vs v15 (vs recorded v15 numbers) ===")
    results = {"reverse": [], "rotate": []}
    for mode in ("reverse", "rotate"):
        for seed in SEEDS:
            results[mode].append(run_changepoint(mode, seed))
    print("\n=== summary: v21 frac of oracle-gap recovered vs plain v10 ===")
    for mode in ("reverse", "rotate"):
        vals = results[mode]
        print(f"{mode}: v21 frac_saved per seed = {[round(v,1) for v in vals]}  mean={np.mean(vals):.1f}%")
